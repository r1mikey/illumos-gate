...
static int mtype4g;
static int mtype16m;
static int mtypetop;		/* index of highest pfn'ed mnoderange */
...
caddr_t
aarch64devmap(pfn_t pf, pgcnt_t pgcnt, uint_t prot)
{
	caddr_t addr;
	caddr_t addr1;
	page_t *pp;
	...
	uint_t flags)
{
	 ...
	 *
	 * XX64 -- is there an ELF_AARCH64_P32_MAXPGSZ or is it the same????
	 */
    ...
    caddr_t userlimit)
{
	...
	/*
	 * Check for the VA hole XXXAARCH64: kernelspace can be protected here
	 */
...
page_coloring_init(uint_t l2_sz, int l2_linesz, int l2_assoc)
{
	...
	if (plat_dr_support_memory()) {
		 ...
		 * for memory hot-add operations on aarch64 platforms.
		 */
...
uint_t
page_create_update_flags_aarch64(uint_t flags)
{
 ...
 * to an alternate mapping.  Unnecessary on Intel / AMD platforms. XXXAARCH64
 */
void
dcache_flushall()
{}
