...
static void aarch64pte_zero(htable_t *dest, uint_t entry, uint_t count);
...
static aarch64pte_t *aarch64pte_access_pagetable(htable_t *ht, uint_t index);
static void aarch64pte_release_pagetable(htable_t *ht);
static aarch64pte_t aarch64pte_cas(htable_t *ht, uint_t entry,
    aarch64pte_t old, aarch64pte_t new);
	...
	htable_t	*shared)
{
	 ...
	 */
	if (need_to_zero)
		aarch64pte_zero(ht, 0, mmu.ptes_per_table);
...
unlink_ptp(htable_t *higher, htable_t *old, uintptr_t vaddr)
{
	...
	aarch64pte_t	expect = MAKEPTP(old->ht_pfn, old->ht_level);
	aarch64pte_t	found;
	...
	found = aarch64pte_cas(higher, entry, expect, 0);
	if (found != expect)
		panic("Bad PTP found=" FMT_PTE ", expected=" FMT_PTE,
		    found, expect);

	/*
	 * We have to INVLPG against an address covered by the inner page table,
	 * as processors have TLB-like caches for non-leaf page table entries.
	 */
	if (!(hat->hat_flags & HAT_FREEING)) {
...
{
	uint_t		entry = htable_va2entry(vaddr, higher);
	aarch64pte_t	newptp = MAKEPTP(new->ht_pfn, new->ht_level);
	aarch64pte_t	found;
	...
	found = aarch64pte_cas(higher, entry, 0, newptp);
	if (found)	/* XXXAARCH64: check the i86pc version of this */
 ...
 *
 * During process exit, we don't bother unlinking the tables from upper level
 * pagetables. They are instead handled in bulk by hat_free_end().
 */
void
...
#if 0
		/*
		 * i86pc does some 32 bit handling here, but we first need
		 * to introduce 32 bit user VA, so punt for now.  Also, this
		 * is from the original port and NOT what x86 does.
		 */
		if (!(base <= vaddr &&
		    vaddr <= (base +
		    HTABLE_NUM_PTES(hat->hat_htable) * LEVEL_SIZE(level)
		    - 1))) {
			return NULL;
		}
#endif
 ...
 *
 * XXXAARCH64: This is a bit iffy, in that we're stradding the TTBR split here
 * needs some thought and investigation.
 */
void
htable_attach(
	hat_t *hat,
	...
	pfn_t pfn)
{
	...
	ASSERT(pp != NULL);
	ASSERT(!PP_ISFREE(pp));			/* XXXAARCH64: original port */
	/* ASSERT(pp->p_lckcnt == 1); */	/* XXXAARCH64: fails - why? */
	ASSERT(PAGE_EXCL(pp));			/* XXXAARCH64: original port */

	/*
	 * Page table pages that were allocated by eboot or
	 ...
	 * XXXAARCH64: this needs a thorough auditing
	 */
...
{
	uint_t e;
	aarch64pte_t found_pte = (aarch64pte_t)0;
	caddr_t pte_ptr;
	caddr_t end_pte_ptr;
	int l = ht->ht_level;
	...
	/*
	 * The following page table scan code knows that the valid bit of a PTE
	 * is in the lowest byte AND that aarch64 is little endian!
	 */
	pte_ptr = (caddr_t)aarch64pte_access_pagetable(ht, 0);
	end_pte_ptr = (caddr_t)PT_INDEX_PTR(pte_ptr, HTABLE_NUM_PTES(ht));
	pte_ptr = (caddr_t)PT_INDEX_PTR((aarch64pte_t *)pte_ptr, e);
	...
	if (va < eaddr && pte_ptr != end_pte_ptr)
		found_pte = GET_PTE((aarch64pte_t *)pte_ptr);
	aarch64pte_release_pagetable(ht);

	/*
	 * No need to deal with the hole on aarch64 as it is architectural and
	 * cannot be mapped by either the lower or upper half translation
	 * tables.
	 */
	...
	uintptr_t eaddr)
{
	...
	while (va < eaddr && va >= *vaddr) {
		ASSERT(!IN_VA_HOLE(va));	/* XXXAARCH64: new in port */
...
htable_init()
{
	 ...
	 * kernels. XXXAARCH64: odd comment, is this a leftover?
	 */
...
htable_e2va(htable_t *ht, uint_t entry)
{
	...
	/*
	 * Unlike amd64 we do not need hole dancing logic, because VA in
	 * the hole architecturally cannot be mapped.  If a VA is above
	 * the hole, it belongs the kas - otherwise it belongs to a user
	 * htable.
	 */
	return (va);
}
...
/*
 * XXXAARCH64:
 * void x86pte_cpu_init(cpu_t *cpu);
 * void x86pte_cpu_fini(cpu_t *cpu);
 *
 * These are a bit suspicious, but I need to get my head around them.  The
 * mutex stuff relates to temporary mapping stuff, which we don't actually
 * need (or want).  Along with this is the hci_mutex.
 *
 * Also the mapin/maput stuff and the PT acquisition and release code.
 */
void
aarch64pte_cpu_init(cpu_t *cpu)
{
	/* nothing */
}

void
aarch64pte_cpu_fini(cpu_t *cpu)
{
	/* nothing */
}

/*
 * Establish a mapping to the pagetable with the given pfn.
 */
static aarch64pte_t *
aarch64pte_access_pagetable(htable_t *ht, uint_t index)
{
	return (aarch64pte_mapin(ht->ht_pfn, index, ht));
}
...
/*ARGSUSED*/
aarch64pte_t *
aarch64pte_mapin(pfn_t pfn, uint_t index, htable_t *ht)
{
	aarch64pte_t *pteptr;
	aarch64pte_t pte = 0;
	aarch64pte_t newpte = 0;
	int x;
	...
	/*
	 * We require kpm to be available at this point
	 */
	ASSERT(kpm_vbase);
 ...
 */
static void
aarch64pte_release_pagetable(htable_t *ht)
{
	aarch64pte_mapout();
}

void
aarch64pte_mapout(void)
{
}
 ...
 */
aarch64pte_t
aarch64pte_get(htable_t *ht, uint_t entry)
{
	aarch64pte_t	pte;
	aarch64pte_t	*ptep;

	ASSERT(entry < mmu.ptes_per_table);
	ptep = aarch64pte_access_pagetable(ht, entry);
	pte = GET_PTE(ptep);
	aarch64pte_release_pagetable(ht);
	return (pte);
}
 ...
 */
aarch64pte_t
aarch64pte_set(htable_t *ht, uint_t entry, aarch64pte_t new, void *ptr)
{
	aarch64pte_t	old;
	aarch64pte_t	prev;
	aarch64pte_t	*ptep;
	...
	/* don't use to invalidate a PTE, see aarch64pte_update */
	ASSERT(new != 0);
	...
	do {
		prev = GET_PTE(ptep);
		n = new;
		if (PTE_ISVALID(n) && ((PTE_TO_PA(prev)) == (PTE_TO_PA(new)))) {
			PTE_CLR(n, PT_ATTR_DBM|PT_SOFT_DBM|PTE_AP_MASK|PTE_AF);
			PTE_SET(n, PTE_GET(prev,
			    PT_ATTR_DBM|PT_SOFT_DBM|PTE_AP_MASK|PTE_AF));
		}
		 ...
		 *
		 * XXXAARCH64: check level stuff here -- see PTE_IS_TABLE
		 */
		if (PTE_IS_TABLE(prev, l)) {
			old = LPAGE_ERROR;
			goto done;
		}
 ...
 */
aarch64pte_t
aarch64pte_cas(htable_t *ht, uint_t entry, aarch64pte_t old, aarch64pte_t new)
{
	aarch64pte_t	pte;
	aarch64pte_t	*ptep;
	...
	boolean_t tlb)
{
	...
	do {
		oldpte = GET_PTE(ptep);
		if (expect != 0 && (PTE_TO_PA(oldpte)) != (PTE_TO_PA(expect)))
			goto done;
		found = CAS_PTE(ptep, oldpte, 0);
	} while (found != oldpte);
	if (tlb && (PTE_IS_DIRTY(oldpte) || PTE_IS_ACCESSED(oldpte)))
	...
	aarch64pte_t new)
{
	...
	if (found == expect) {
		...
		/*
		 * XXXAARCH64:
		 * i86pc has a pile of code here to deal with late writes via
		 * a stale TLB entry before shootdown completes.  We might need
		 * to reinstate this logic here, but we can only do so once
		 * we've figured out the modified bits stuff and DBM.
		 */
	}
	aarch64pte_release_pagetable(ht);
	return (found);
 ...
 *
 * Ought to be used once we slew the rest of this stuff to look like x86
 */
void
aarch64pte_copy(htable_t *src, htable_t *dest, uint_t entry, uint_t count)
{
	caddr_t	src_va;
	caddr_t dst_va;
	size_t size;
 ...
 */
static void
aarch64pte_zero(htable_t *dest, uint_t entry, uint_t count)
{
	caddr_t dst_va;
	size_t size;
	...
	dst_va = (caddr_t)aarch64pte_access_pagetable(dest, entry);

	size = count << mmu.pte_size_shift;
	ASSERT(size > BLOCKZEROALIGN);
	bzero(dst_va, size);
	aarch64pte_release_pagetable(dest);
}
