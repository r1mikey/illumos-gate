...
#define	CHECK_RANGE(__va, __sz, __s, __e)			\
	if ((__va) + (__sz) >= (__s) && (__va) < (__e))		\
		return ((__va) < (__s) ? (__s) - (__va) : 0)

	/*
	 * Check 64 bit toxic range.
	 */
	if (toxic_addr != 0)
		CHECK_RANGE(va, sz, toxic_addr, toxic_addr + toxic_size);

	CHECK_RANGE(va, sz, BOOTLOADER_DATA_BASE,
	    BOOTLOADER_DATA_BASE + BOOTLOADER_DATA_SIZE);
	CHECK_RANGE(va, sz, DBG2_BASE, DBG2_BASE + DBG2_SIZE);
	CHECK_RANGE(va, sz, FRAMEBUFFER_BASE,
	    FRAMEBUFFER_BASE + FRAMEBUFFER_MAX_SIZE);
	CHECK_RANGE(va, sz, UEFI_RUNTIME_BASE,
	    UEFI_RUNTIME_BASE + UEFI_RUNTIME_MAX_SIZE);

	/*
	 * avoid any Virtual Address hole
	 */
	CHECK_RANGE(va, sz, hole_start, hole_end);
