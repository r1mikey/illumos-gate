 ...
 * Copyright 2017 Michael van der Westhuizen
 */
 ...
 */
extern htable_t *htable_getpte(struct hat *, uintptr_t, uint_t *,
	aarch64pte_t *, level_t);
 ...
 *
 */
extern aarch64pte_t htable_walk(struct hat *hat, htable_t **ht, uintptr_t *va,
	uintptr_t eaddr);

#define	HTABLE_WALK_TO_END ((uintptr_t)-1)

/*
 * Utilities to convert between virtual addresses and page table entry indeces.
 */
 ...
 *
 * Note that all accesses except aarch64pte_copy() and aarch64pte_zero() are
 * atomic.
 */
extern void	aarch64pte_cpu_init(cpu_t *);
extern void	aarch64pte_cpu_fini(cpu_t *);

extern aarch64pte_t aarch64pte_get(htable_t *, uint_t entry);

/*
 * aarch64pte_set returns LPAGE_ERROR if it's asked to overwrite a page table
 * link with a large page mapping.
 */
#define	LPAGE_ERROR (-(aarch64pte_t)1)
extern aarch64pte_t aarch64pte_set(htable_t *, uint_t entry,
	aarch64pte_t new, void *);

extern aarch64pte_t aarch64pte_inval(htable_t *ht, uint_t entry,
	aarch64pte_t old, aarch64pte_t *ptr, boolean_t tlb);

extern aarch64pte_t aarch64pte_update(htable_t *ht, uint_t entry,
	aarch64pte_t old, aarch64pte_t new);

extern void aarch64pte_copy(htable_t *src, htable_t *dest, uint_t entry,
	uint_t cnt);

/*
 * access to a pagetable knowing only the pfn
 */
extern aarch64pte_t *aarch64pte_mapin(pfn_t, uint_t, htable_t *);
extern void aarch64pte_mapout(void);
