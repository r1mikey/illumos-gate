...
hment_alloc()
{
	...
	if (hm == NULL) {
		...
		/*
		 * we're in dire straits, try the reserve
		 */
		if (hm == NULL)
			hm = hment_get_reserve();
 ...
 *
 * must already have done an aarch64_hm_enter()
 */
hment_t *
 ...
 * Finds the mapping list entry with the given aarch64pte_t and
