...
#ifndef	_EBOOT_XBOOT_H
#define	_EBOOT_XBOOT_H
...
#define	DBG_P(p)	do { if (prom_debug)	\
	eboot_printf p;				\
...
#define	DBG(x)	do { if (prom_debug) {					\
	eboot_printf("%s is 0x%" PRIx64 "\n", #x, (uint64_t)(x));	\
	_NOTE(CONSTANTCONDITION)					\
	} } while (0)

extern void eboot_halt(void) __NORETURN;
