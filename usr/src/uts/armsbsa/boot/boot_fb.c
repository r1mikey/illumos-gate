...
/*
 * [de]boot and early kernel needs simple putchar(int) interface to implement
...
/*
 * I'm sorry, I'm a bad person who copies snippets of code around to avoid
 * teasing unexpected dependencies out of a large codebase.
 */
static int
xbi_fb_init_ffs(uintmax_t bits)
{
        int i;
...
xbi_fb_init(struct xboot_info *xbi, bcons_dev_t *bcons_dev)
{
...
#if !defined(_BOOT)
	/* For early kernel, we get cursor position from [de]boot. */
	...
	xbi_init_font(xbi);
	efifb = (struct efi_fb *)(uintptr_t)xbi_fb->framebuffer;
	if (efifb == NULL)
		return (B_FALSE);

	fb_info.paddr = efifb->fb_addr;
	fb_info.screen.x = efifb->fb_width;
	fb_info.screen.y = efifb->fb_height;
	fb_info.depth = xbi_fb_init_fls(efifb->fb_mask_red |
	    efifb->fb_mask_green |
	    efifb->fb_mask_blue |
	    efifb->fb_mask_reserved);
	fb_info.pitch = efifb->fb_stride * (fb_info.depth >> 3);
	fb_info.bpp = P2ROUNDUP(fb_info.depth, 8) >> 3;
	fb_info.fb_size = efifb->fb_size;
	...
	pos = xbi_fb_init_ffs(efifb->fb_mask_red);
	if (pos != 0)
		pos--;
	fb_info.rgb.red.size = xbi_fb_init_fls(efifb->fb_mask_red >> pos);
	fb_info.rgb.red.pos = pos;

	pos = xbi_fb_init_ffs(efifb->fb_mask_green);
	if (pos != 0)
		pos--;
	fb_info.rgb.green.size = xbi_fb_init_fls(efifb->fb_mask_green >> pos);
	fb_info.rgb.green.pos = pos;

	pos = xbi_fb_init_ffs(efifb->fb_mask_blue);
	if (pos != 0)
		pos--;
	fb_info.rgb.blue.size = xbi_fb_init_fls(efifb->fb_mask_blue >> pos);
	fb_info.rgb.blue.pos = pos;
...
boot_fb_shadow_init(bootops_t *bops)
{
	...
	if (fb_info.fb_size > SHADOWFB_MAX_SIZE)
		return;			/* ETOOBAD */

	fb_info.shadow_fb = (uint8_t *)bops->bsys_alloc(NULL,
	    (caddr_t)SHADOWFB_BASE, fb_info.fb_size, MMU_PAGESIZE);
...
#if defined(_BOOT)
	/*
	 * Being called from [de]boot, we can have cursor terminal
