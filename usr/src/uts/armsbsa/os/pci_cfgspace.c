 ...
 *
 * The Arm Base System Architecture (BSA) 1.0 (DEN0094A), rule PCI_IN_01,
 * states:
 *	Systems must map memory space to PCI Express Configuration Space, using
 *	the PCI Express Enhanced Configuration Access Mechanism (ECAM).
 *
 * To support this goal, MCFG is q required ACPI table in the Base Boot
 * Requirements when PCIe is supported.
 */
...
pci_check(void)
{
	...
	/*
	 * In Arm we are presently only set up to support UEFI systems.
	 */
	if (BOP_GETPROPLEN(bootops, "efi-systab") <= 0)
		return (FALSE);

	/*
	 * For UEFI systems we assume some reasonable defaults and scan
	 * all possible buses.
	 */
	pci_bios_mech = 1;
	pci_bios_vers = 0;
	...
	/*
	 * This is from x86 code, and allows overriding the mechanism in
	 * /etc/system.  This doesn't make a ton of sense on Arm platforms,
	 * so we simply return false if the operator attempts an override that
	 * we can't handle.
	 */
...
/*
 * For Arm-based platforms we assume that mechanism #1 is available and works
 * correctly.
 */
static int
