...
startup_build_mem_nodes(struct memlist *list)
{
	...
	/*
	 * sun4v has some pretty cool MPO stuff that ties into lgroups - we
	 * should see about picking that up.  For now, we take the generic path.
	 *
	 * Note that i86pc also has some pretty cool stuff here.
	 *
	 * for aarch64 we should have all of this goodness, but as none of this
	 * is particularly ARM implementation-specific we should hoist it to the
	 * aarch64 layer (much of aarch64 is architectural).
	 *
	 * XXXAARCH64: memnodes startup needs to be fleshed out
	 */
