...
/* XXXAARCH64: FreeBSD loader compat */
uint64_t __start_set_modmetadata_set = 0;
uint64_t __stop_set_modmetadata_set = 0;

EFI_RUNTIME_SERVICES64 *efirt;
...
mdboot(int cmd, int fcn, char *mdep, boolean_t invoke_cb)
{
	...
	if (fcn == AD_HALT) {
		...
		stop_other_cpus();
		/* XXXAARCH64: needs fleshing out prom_power_off(); */
	} else {
		stop_other_cpus();
		prom_reboot("");
	}
...
{
	return (plat_dr_check_capability(PLAT_DR_FEATURE_CPU));
}

boolean_t
plat_dr_support_memory(void)
{
	return (plat_dr_check_capability(PLAT_DR_FEATURE_MEMORY));
}
...
{
	if (efirt == NULL) {
		panic("reset: UEFI runtime services pointer not yet set");
	} else if (efirt->ResetSystem == NULL) {
		panic("reset: UEFI runtime services ResrtSystem function "
		    "pointer not yet set");
	} else {
