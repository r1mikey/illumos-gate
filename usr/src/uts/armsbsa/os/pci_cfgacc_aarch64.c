...
static boolean_t
pci_cfgacc_check_mmio(pci_cfgacc_req_t *req)
{
	uint8_t bus;
	...
	if (mcfg_mem_base != 0 || bus >= mcfg_bus_start || bus <= mcfg_bus_end)
		return B_TRUE;

	cmn_err(CE_WARN, "illegal PCI request: MCFG base = 0x%lx, bus = %u",
	    mcfg_mem_base, bus);
	return B_FALSE;
}
...
pci_cfgacc_acc(pci_cfgacc_req_t *req)
{
	...
	pci_cfgacc_check_mmio(req);

	if (pci_cfgacc_check_mmio(req) &&
	    pci_cfgacc_valid(req, PCIE_CFG_SPACE_SIZE))
		pci_cfgacc_mmio(req);
}
