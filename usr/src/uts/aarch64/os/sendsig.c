...
sendsig(int sig, k_siginfo_t *sip, void (*hdlr)())
{
	 ...
	 */
#ifdef DEBUG
	if (lwp->lwp_pcb.pcb_fpu.fpu_flags & FPU_KERNEL) {
		panic("sendsig: FPU_KERNEL set on signal delivery, "
		    "fpu_flags=0x%x, t_flag=0x%x, pid=%d",
		    lwp->lwp_pcb.pcb_fpu.fpu_flags,
		    curthread->t_flag, p->p_pidp->pid_id);
	}
#endif
	VERIFY(!(lwp->lwp_pcb.pcb_fpu.fpu_flags & FPU_KERNEL));
