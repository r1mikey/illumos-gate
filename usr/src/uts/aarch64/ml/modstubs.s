 ...
 *	real function. On aarch64 we can't tell how many arguments are there
 ...
 *	      loaded.
 *
 *	MAXNARG must be an even number due to stack alignment requirements
 *	on aarch64.
 */
#define	MAXNARG		10
...
#if 0
/*
 * Stubs for acpica - need proper hw reduced support
 */
#ifndef ACPICA_MODULE
	MODULE(acpica,misc);
	STUB(acpica, AcpiOsReadPort, nomod_minus_one) ;
	STUB(acpica, AcpiOsWritePort, nomod_minus_one) ;
	STUB(acpica, AcpiInstallNotifyHandler, nomod_minus_one) ;
	STUB(acpica, AcpiRemoveNotifyHandler, nomod_minus_one) ;
	STUB(acpica, AcpiEvaluateObject, nomod_minus_one) ;
	STUB(acpica, AcpiEvaluateObjectTyped, nomod_minus_one) ;
	STUB(acpica, AcpiOsFree, nomod_minus_one) ;
	STUB(acpica, acpica_get_handle_cpu, nomod_minus_one) ;
	STUB(acpica, acpica_get_global_FADT, nomod_minus_one) ;
	STUB(acpica, acpica_write_cpupm_capabilities,
	    nomod_minus_one)		       ;
	STUB(acpica, __acpi_wbinvd, nomod_minus_one) ;
	STUB(acpica, acpi_reset_system, nomod_minus_one) ;
	END_MODULE(acpica);
#endif
#endif
...
/*
 * XXXAARCH64: This is from the original port - interesting, and probably a
 * good idea.
 */
#ifndef	PLATMOD_MODULE
	MODULE(platmod,drv);
