...
pnode_t prom_rootnode(void)
{
	return (prom_ops->po_rootnode());
}

pnode_t prom_nextnode(pnode_t nodeid)
{
	return (prom_ops->po_nextnode(nodeid));
}

pnode_t prom_childnode(pnode_t nodeid)
{
	return (prom_ops->po_childnode(nodeid));
}

pnode_t prom_findnode_byname(pnode_t n, char *name)
{
	return (prom_ops->po_findnode_byname(n, name));
}

pnode_t prom_chosennode(void)
{
	return (prom_ops->po_chosennode());
}

pnode_t prom_optionsnode(void)
{
	return (prom_ops->po_optionsnode());
}

pnode_t prom_finddevice(const char *path)
{
	return (prom_ops->po_finddevice(path));
}

pnode_t prom_alias_node(void)
{
	return (prom_ops->po_alias_node());
}

void prom_pathname(char *buf)
{
