...
extern const prom_ops_t prom_fdt_ops;
extern const prom_ops_t *prom_ops;

#if defined(DEBUG)
#define	VERIFY_FDT()	do {					\
	if (prom_fdtp == NULL || prom_ops != &prom_fdt_ops) {	\
		prom_panic("PROM: FDT pointer is not set");	\
	}							\
} while (0)
#else
#define	VERIFY_FDT()
...
prom_fdt_getprop(pnode_t nodeid, const char *name, caddr_t value)
{
	...
	if (prop == NULL) {
		if (strcmp(name, "name") == 0) {
			const char *name_ptr =
			    fdt_get_name(prom_fdtp, offset, &len);
...
prom_fdt_getproplen(pnode_t nodeid, const char *name)
{
	...
	offset = fdt_node_offset_by_phandle(prom_fdtp, (pnode_t)nodeid);
	if (offset < 0)
		return (-1);
	...
	const struct fdt_property *prop =
	    fdt_get_property(prom_fdtp, offset, name, &len);
...
prom_fdt_finddevice(const char *device)
{
	...
	offset = fdt_path_offset(prom_fdtp, device);
	if (offset < 0)
		return (OBP_BADNODE);
...
prom_fdt_chosennode(void)
{
	...
	node = prom_fdt_finddevice("/chosen");
	if (node != OBP_BADNODE)
		return (node);
	return (OBP_NONODE);
}
...
prom_fdt_optionsnode(void)
{
	...
	node = prom_fdt_finddevice("/options");
	if (node != OBP_BADNODE)
		return (node);
	return (OBP_NONODE);
}
...
prom_fdt_nextprop(pnode_t nodeid, char *name, char *next)
{
	...
	*next = '\0';
	...
	data = fdt_get_property_by_offset(prom_fdtp, offset, NULL);
	strlcpy(next, (char *)fdt_string(prom_fdtp,
	    fdt32_to_cpu(data->nameoff)), OBP_MAXPROPNAME);
	return (next);
}
    ...
    pnode_t nodeid, const char *name, caddr_t value, int len)
{
	...
	prop_len = prom_fdt_getproplen(nodeid, name);
...
/*
 * PROM FDT initialisation
 *
 * If the cookie argument is non-NULL and represents a valid DTB, save the
 * cookie as the FDT and set the PROM operations pointer to point to the
 * FDT operations object.
 */
void
...
/*
 * B_TRUE iff the FDT pointer is set.
 */
boolean_t
prom_fw_is_fdt(void)
{
...
/*
 * Retrieve the FDT pointer, for users who need to directly interact with the
 * FDT using libfdt.
 */
void *
...
/*
 * Set the FDT pointer to a new memory address. Used during boot to point to a
 * new mapping before the boot pages are unmapped.
 */
int
 ...
 *
 * Use of these functions is discouraged and banned outside of startup code
 * that is intended to be common between FDT and ACPI.
 */
...
prom_fdt_findnode_by_phandle(phandle_t phandle)
{
	...
	offset = fdt_node_offset_by_phandle(prom_fdtp, phandle);
	if (offset < 0)
		return (-1);
	return ((pnode_t)phandle);
}
 ...
 *
 * These shouldn't really be a part of the PROM at all. As with the other
 * supporting functions, you should avoid these wherever DDI functions could
 * be used instead.
 */
...
prom_fdt_get_prop_index(pnode_t node, const char *prop_name, const char *name)
{
	...
	len = prom_fdt_getproplen(node, prop_name);
	if (len > 0) {
		char *prop = __builtin_alloca(len);
		prom_fdt_getprop(node, prop_name, prop);
		int offset = 0;
		int index = 0;
		while (offset < len) {
...
prom_fdt_get_prop_int(pnode_t node, const char *name, int def)
{
	...
	while (node > 0) {
		int len = prom_fdt_getproplen(node, name);
		if (len == sizeof (int)) {
			int prop;
			prom_fdt_getprop(node, name, (caddr_t)&prop);
			value = ntohl(prop);
			break;
		}
		if (len > 0) {
...
prom_fdt_get_prop_u64(pnode_t node, const char *name, uint64_t def)
{
	...
	if (node > 0 && prom_fdt_getproplen(node, name) == sizeof (uint64_t)) {
		prom_fdt_getprop(node, name, (caddr_t)&prop);
		value = ntohll(prop);
	}
...
prom_fdt_get_prop_u32(pnode_t node, const char *name, uint32_t def)
{
	...
	if (node > 0 && prom_fdt_getproplen(node, name) == sizeof (uint32_t)) {
		prom_fdt_getprop(node, name, (caddr_t)&prop);
		value = ntohl(prop);
	}
...
prom_fdt_get_clock(pnode_t node, int index, struct prom_hwclock *clock)
{
	...
	len = prom_fdt_getproplen(node, "clocks");
	if (len <= 0)
		return (-1);

	uint32_t *clocks = __builtin_alloca(len);
	prom_fdt_getprop(node, "clocks", (caddr_t)clocks);
    ...
    const char *name, struct prom_hwclock *clock)
{
	...
	index = prom_fdt_get_prop_index(node, "clock-names", name);
	if (index >= 0)
...
prom_fdt_get_size_cells(pnode_t node)
{
	...
	return (prom_fdt_get_prop_int(
	    prom_fdt_parentnode(node), "#size-cells", 2));
}
...
prom_fdt_get_reg_bounds(pnode_t node, int index, uint64_t *base, uint64_t *size)
{
	...
	len = prom_fdt_getproplen(node, "reg");
	if (len <= 0)
		return (-1);

	uint32_t *regs = __builtin_alloca(len);
	prom_fdt_getprop(node, "reg", (caddr_t)regs);
...
prom_fdt_get_reg_address(pnode_t node, int index, uint64_t *reg)
{
	...
	while (parent > 0) {
		...
		int len = prom_fdt_getproplen(parent, "ranges");
		if (len <= 0) {
			parent = prom_fdt_parentnode(parent);
			continue;
		}
		...
		int size_cells =
		    prom_fdt_get_prop_int(parent, "#size-cells", 2);
		...
		prom_fdt_getprop(parent, "ranges", (caddr_t)ranges);
		int ranges_cells =
