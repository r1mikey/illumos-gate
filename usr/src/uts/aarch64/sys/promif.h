...
extern	int		prom_bounded_getprop(pnode_t nodeid, const char *name,
...
extern	void		prom_power_off(void) __NORETURN;
...
#if defined(_BOOT)
extern	int		prom_fdt_setprop(pnode_t nodeid, const char *name,
...
extern	int		prom_fdt_get_prop_int(pnode_t node,
    const char *name, int def);
extern	uint64_t	prom_fdt_get_prop_u64(pnode_t node,
    const char *name, uint64_t def);
extern	uint32_t	prom_fdt_get_prop_u32(pnode_t node,
    const char *name, uint32_t def);
extern	int		prom_fdt_get_reg(pnode_t node,
    int index, uint64_t *base);
...
extern	int		prom_fdt_get_clock_by_name(pnode_t node,
...
extern	pnode_t		prom_fdt_find_compatible(pnode_t node,
    const char *compatible);
extern	boolean_t	prom_fdt_has_compatible(const char *compatible);
extern	void		prom_fdt_walk(void(*func)(pnode_t, void*), void *arg);
extern	int		prom_fdt_get_reg_address(pnode_t node,
    int index, uint64_t *reg);
extern	int		prom_fdt_get_reg_size(pnode_t node,
    int index, uint64_t *regsize);
extern	boolean_t	prom_fdt_node_has_property(pnode_t nodeid,
    const char *name);

/*
 * FDT pointer manipulation
 */
extern	void *		prom_fdt_get_fdtp(void);
extern	int		prom_fdt_set_fdtp(void *fdtp);

/*
 * Firmware interface query functions
 */
