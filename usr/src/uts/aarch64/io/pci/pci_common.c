    ...
    ddi_intr_op_t intr_op, ddi_intr_handle_impl_t *hdlp, void *result)
{
	...
	if (hdlp->ih_type == DDI_INTR_TYPE_FIXED) {
		...
		/*
		 * PCI INTx is always level-triggered.  The GIC
		 * honestly reports EDGE|LEVEL for SPIs, but EDGE
		 * is not valid for INTx -- clear it.
		 */
    ...
    ddi_intr_op_t intr_op, ddi_intr_handle_impl_t *hdlp)
{
	...
	if (hdlp->ih_type == DDI_INTR_TYPE_FIXED) {
		 ...
		 * Fall back to the interrupt controller if it
    ...
    ddi_intr_handle_impl_t *hdlp, void *result)
{
	...
	if (hdlp->ih_type == DDI_INTR_TYPE_FIXED) {
		...
		(void) i_ddi_intr_ops(pdip, rdip, DDI_INTROP_GETPENDING,
		    hdlp, (void *)&gic_status);

		*(int *)result = pci_status | gic_status;
