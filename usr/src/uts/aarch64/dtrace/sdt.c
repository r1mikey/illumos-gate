...
#define	SDT_PATCHVAL	0xd4200220	// brk #0x11
...
sdt_invop(uintptr_t addr, uintptr_t *stack, uintptr_t rval)
{
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == addr) {
			...
			dtrace_probe(sdt->sdp_id, rp->r_x0, rp->r_x1, rp->r_x2, rp->r_x3, rp->r_x4);
...
sdt_write_instruction(sdt_instr_t *inst, sdt_instr_t val)
{
	...
	*(sdt_instr_t* )((uintptr_t)vaddr + ((uintptr_t)inst & MMU_PAGEOFFSET)) = val;
