...
/*
 * EFI memory map header, as passed by loader via dboot. This is a wrapper
 * over the data returned by EFI_BOOT_SERVICES.GetMemoryMap(), and not
 * a structure defined in any UEFI specification.
 *
 * Immediately followed in memory by an array of EFI_MEMORY_DESCRIPTOR
 * entries, each descriptor_size bytes long.  Use efi_mmap_next() to
 * iterate, as descriptor_size may exceed sizeof (EFI_MEMORY_DESCRIPTOR)
 * in future UEFI revisions.
 */
