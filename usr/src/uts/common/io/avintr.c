    ...
    caddr_t arg1, caddr_t arg2, uint64_t *ticksp, dev_info_t *dip)
{
	 ...
	 *
	 * On aarch64, MAX_VECT < the INTID space (LPIs can be >=8192),
	 * so multiple vectors hash to the same bucket.  We must only
	 * check for IPL conflicts among entries with the same vector,
	 * not all entries in the bucket.
	 */
