...
/*
 * Processor Specific Module subsystem implementation
 */
...
/*
 * From usr/src/uts/common/os/modsysfile.c
 */
...
int
psm_mod_fini(void **handlepp, struct psm_info *infop __unused)
{
	struct modlinkage **modlpp = (struct modlinkage **)handlepp;
	int	status;
...
psm_debugger(void)
{
	/* XXXARM: return ((boothowto & RB_DEBUG) != 0); */
	return (0);
}

/*
 * mod_ops implementation functions
 */

static int
mod_installpsm(struct modlpsm *modl, struct modlinkage *modlp __unused)
{
	struct psm_sw *swp;
...
static int
mod_removepsm(struct modlpsm *modl, struct modlinkage *modlp __unused)
{
	struct psm_sw *swp;
...
static int
mod_infopsm(struct modlpsm *modl, struct modlinkage *modlp __unused, int *p0)
{
	*p0 = (int)modl->psm_swp->psw_infop->p_owner;
	return (0);
}

#define	DEFAULT_PSM_MODULE	"armbsa"
...
/*
 * Declared in sys/smp_impldefs.h (why?)
 */
void
psm_modload(void)
{
