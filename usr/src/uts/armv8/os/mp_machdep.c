 ...
 *
 * This is the fallback when no idle framework overrides cpu_halt_impl.
 */
static void
cpu_halt_wfi(void)
{
 ...
 *
 * A firmware-specific cpuidle module can override this with a function that
 * selects an idle state and enters it (WFI or PSCI CPU_SUSPEND).  This
 * implementation hook is called with interrupts disabled from the inner loop
 * of cpu_halt.
 */
