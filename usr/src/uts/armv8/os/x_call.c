...
/*
 * Lock-free per-CPU cross-call implementation for aarch64.
 *
 * Direct port of the Joyent i86pc xcall redesign.  Each CPU owns a set
 * of preallocated xc_msg_t slots in its free list.  Initiators allocate
 * a message from their OWN free list, fill it in (command + slave id),
 * CAS it onto the target's msgbox, then send an IPI.  The receiver
 * drains its msgbox, executes the function (read from the master's
 * xc_data), and sends the message back to the master's msgbox as DONE.
 * The master processes DONE messages in its own xc_serv() call and
 * returns them to its free list.
 *
 * State transitions for xc_command ("->" in slave, "=>" in master):
 *
 ...
 *
 * Memory ordering:
 *   aarch64 atomic_cas_ptr/atomic_cas_32/atomic_dec_32 provide
 *   acquire-release ordering.  ACQ_REL suffices for the CAS-based
 *   queue operations.  xc_priority_common() requires an explicit
 *   membar_enter() (DMB ISH) after writing xc_priority_data and
 *   before setting xc_priority_set bits, so targets observe the
 *   data before the set bit.
 *
 * IPI dispatch:
 *   armv8 send_dirint() takes an SGI INTID, not a PIL.  We use
 *   IRQ_IPI_HI (the cross-call SGI) directly.
 */
...
xc_insert(void *queue, xc_msg_t *msg)
{
	 ...
	 * the xc_master CPU's xc_free queue.
	 */
...
static uint_t xc_ipi_registered = 0;
...
xc_init_cpu(struct cpu *cpup)
{
	...
	/*
	 * Register IPI handlers on first call (BSP only).
	 */
	if (!xc_ipi_registered) {
		add_avintr((void *)NULL, XC_HI_PIL, xc_serv, "xc_intr",
...
xc_serv(caddr_t arg1 __unused, caddr_t arg2 __unused)
{
	...
	while (mcpup->xc_work_cnt != 0) {
		...
		/*
		 * WAITING messages are collected by the master until all
		...
		/*
		 * CALL messages do the function and then, like RELEASED,
...
/*
 * The poke handler does nothing; it exists solely so that a CPU blocked
 * in a tight loop (e.g. mutex_enter spin) gets pulled out by the IPI.
 */
static uint_t
xc_poke(caddr_t arg0 __unused, caddr_t arg1 __unused)
{
	...
	uint_t command)
{
	...
	for (c = 0; c < max_ncpus; ++c) {
		...
		/*
		 * Increment the target CPU work count then insert the
		 * message in the target msgbox.  If I post the first
		 * bit of work for the target to do, send an IPI to the
		 * target CPU.
		 *
		 * Note: armv8 send_dirint() takes an SGI INTID, not a
		 * PIL number.  We use IRQ_IPI_HI directly.
		 */
	...
	ulong_t *set)
{
	...
	for (c = 0; c < max_ncpus; ++c) {
		...
		/*
		 * Some CPU did not respond to a previous priority request.
		 * It's probably deadlocked with interrupts blocked or some such
	...
	/*
	 * On aarch64, we need an explicit barrier here to ensure targets
	 * observe the xc_priority_data stores before the xc_priority_set
	 * bit.  x86 LOCK-prefixed instructions provide this implicitly,
	 * but aarch64 atomics only guarantee acquire-release ordering.
	 */
	membar_enter();
