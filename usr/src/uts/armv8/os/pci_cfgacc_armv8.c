...
pci_cfgacc_map(paddr_t phys_addr)
{
	if (khat_running) {
		...
		panic("pci_cfgacc_map: khat is not running");
#if 0
...
#if 0
/*
 * There's some nice code in SPARC that looks at the bus, which
 * is now we're going to need to look at multiple segments and
 * how we'll need to integrate SMCCC PCI.
 */
static void
