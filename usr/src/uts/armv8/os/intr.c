...
do_interrupt(struct regs *rp)
{
	...
	if (psm_intr_enter(
	    oldipl, &ack, &vector, &newipl, &spurious) != PSM_SUCCESS)
		panic("failed to enter irq handling");
