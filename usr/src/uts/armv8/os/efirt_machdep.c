...
/*
 * EFI Runtime Services support for AArch64.
 *
 ...
 *   - ASID 1 is permanently reserved for the EFI page table.
 *   - A sleep mutex serializes all RT calls (UEFI RT is not
 *     reentrant).
 *   - kernel_fpu_begin/end bracket RT calls because the UEFI
 *     calling convention permits FP/SIMD register use.
 *   - Firmware runs on a dedicated stack (DEFAULTSTKSZ = 20 KB)
 *     to avoid kernel stack overflow from greedy firmware.
 *   - on_trap(OT_DATA_ACCESS) provides fault recovery for both
 *     data aborts and instruction aborts from EL1 (the ARM64 trap
 *     handler dispatches both through the same on_trap check).
 *     Per-call recovery: a fault returns EFI_DEVICE_ERROR to the
 *     caller, but RT services remain available for subsequent calls.
 *
 ...
 * may reap htable pages under memory pressure, but EFI page tables
 ...
 * Bits are cleared dynamically when a service returns EFI_UNSUPPORTED.
 */
static uint32_t		efirt_supported = EFI_RT_SUPPORTED_ALL;

/*
 * Assembly trampoline: switches to the EFI dedicated stack, shuffles
 * arguments, calls the firmware function, and switches back.
 * Defined in armv8/ml/efirt_call.S.
 */
...
/*
 * EFI Runtime Services function pointers (physical addresses).
...
{
	pte_t pte = PTE_PAGE | PTE_AF | PTE_SH_INNER | PTE_NG | PTE_UXN;
...
/*
 * Allocate a zeroed page-table page.  These are never freed -
 * the EFI page table must be permanently pinned.
 */
static pte_t *
...
/*
 * Return the physical address of a kernel virtual address.
 */
static uint64_t
efirt_kva_to_pa(void *va)
{
	return (pfn_to_pa(hat_getpfnum(kas.a_hat, (caddr_t)va)));
}

/*
 * Install a single 4KB page mapping in the EFI L0 page table.
 * Allocates intermediate L1/L2/L3 tables as needed.
 *
 * 4-level, 4KB granule, 48-bit VA (identity map: VA == PA):
 *   L0 index: bits [47:39]  (512GB per entry)
 *   L1 index: bits [38:30]  (1GB per entry)
 *   L2 index: bits [29:21]  (2MB per entry)
 *   L3 index: bits [20:12]  (4KB per entry)
 */
static void
efirt_map_page(pte_t *l0, uint64_t pa, pte_t pte_attr)
{
	uint_t l0i = (pa >> 39) & 0x1FF;
	uint_t l1i = (pa >> 30) & 0x1FF;
	uint_t l2i = (pa >> 21) & 0x1FF;
	uint_t l3i = (pa >> 12) & 0x1FF;
	pte_t *l1, *l2, *l3;

	/* L0 -> L1 */
	if (!PTE_ISVALID(l0[l0i])) {
		l1 = efirt_alloc_ptpage();
		l0[l0i] = efirt_kva_to_pa(l1) | PTE_TABLE;
	} else {
		l1 = (pte_t *)pa_to_kseg(l0[l0i] & PTE_PFN_MASK);
	}

	/* L1 -> L2 */
	if (!PTE_ISVALID(l1[l1i])) {
		l2 = efirt_alloc_ptpage();
		l1[l1i] = efirt_kva_to_pa(l2) | PTE_TABLE;
	} else {
		l2 = (pte_t *)pa_to_kseg(l1[l1i] & PTE_PFN_MASK);
	}

	/* L2 -> L3 */
	if (!PTE_ISVALID(l2[l2i])) {
		l3 = efirt_alloc_ptpage();
		l2[l2i] = efirt_kva_to_pa(l3) | PTE_TABLE;
	} else {
		l3 = (pte_t *)pa_to_kseg(l2[l2i] & PTE_PFN_MASK);
	}

	/* L3 page entry */
	l3[l3i] = (pa & PTE_PFN_MASK) | pte_attr;
}
 ...
 * around well-known firmware bugs - notably on Ampere platforms -
 * where firmware assumes the OS will always call SVAM and behaves
 * incorrectly if it is not called.
 *
 * This function is called once during efirt_init(), after the EFI
 * page table, dedicated stack, and FPU state are ready, but before
 * RT function pointers are extracted.  This ordering is critical:
 * firmware may relocate its internal pointers during SVAM, so the
 * RT function pointer table must be read after SVAM returns.
 *
    ...
    EFI_RUNTIME_SERVICES64 *rt)
{
	...
	status = efirt_call_asm(svam_func,
	    ...
	    (uint64_t)EFI_MEMORY_DESCRIPTOR_VERSION,
...
leave:
	...
	if (status == EFI_SUCCESS) {
		cmn_err(CE_CONT,
		    "?EFI RT: SetVirtualAddressMap succeeded "
		    "(identity map)\n");
	} else {
		cmn_err(CE_WARN,
...
efirt_init(void)
{
	...
	/*
	 * Allocate the L0 page table and identity-map all
	 * EFI_MEMORY_RUNTIME regions.  Page table pages are
	 * allocated via kmem_zalloc() and never freed.
	 */
	efirt_l0 = efirt_alloc_ptpage();
	...
	for (int i = 0; i < ndesc;
		...
		for (uint64_t p = 0; p < npages; p++)
			efirt_map_page(efirt_l0, pa + p * MMU_PAGESIZE,
			    pte_attr);
 ...
 * FPU state, and switches TTBR0 to the EFI 1:1 identity map.
...
efirt_arch_leave(void)
{
	 ...
	 * walks are enabled against the EFI page table after we've
	 * logically left EFI context.
	 */
	write_tcr(efirt_saved_tcr);
	write_ttbr0(efirt_saved_ttbr0);
	isb();
 ...
 *
 * Acquires the RT lock, enters EFI context (TTBR0 switch, FPU save),
 ...
 *
 * Returns the EFI status from firmware, or EFI_DEVICE_ERROR on fault.
    ...
    uint64_t a3, uint64_t a4)
{
	...
	if (on_trap(&otd, OT_DATA_ACCESS) != 0) {
		 ...
		 * before we switched to the EFI stack), so the dedicated
		 * EFI stack is cleanly abandoned.
		 */
		no_trap();
		efirt_arch_leave();
 ...
 */
uint64_t
efi_get_time(EFI_TIME *t, EFI_TIME_CAPABILITIES *caps)
{
	uint64_t status;
	...
	status = efirt_call_rt(efirt_get_time,
	    (uint64_t)t, (uint64_t)caps, 0, 0, 0);
 ...
 * (UEFI Specification 2.10, Section 8.3).
 *
 * enabled and pending are UEFI BOOLEAN (uint8_t) pointers.
 * t receives the current alarm setting.
 */
uint64_t
efi_get_wakeup_time(uint8_t *enabled, uint8_t *pending,
    EFI_TIME *t)
{
	uint64_t status;
	...
	if (!efirt_is_active())
		return (EFI_UNSUPPORTED);
	if (!efirt_is_supported(EFI_RT_SUPPORTED_GET_WAKEUP_TIME))
		return (EFI_UNSUPPORTED);

	status = efirt_call_rt(efirt_get_wakeup_time,
	    (uint64_t)enabled, (uint64_t)pending,
	    (uint64_t)t, 0, 0);
...
/*
 * Set or clear the wakeup alarm
 * (UEFI Specification 2.10, Section 8.3).
 *
 * enable is a UEFI BOOLEAN.  t may be NULL when clearing.
 */
uint64_t
efi_set_wakeup_time(uint8_t enable, EFI_TIME *t)
{
	uint64_t status;

	if (!efirt_is_active())
		return (EFI_UNSUPPORTED);
	if (!efirt_is_supported(EFI_RT_SUPPORTED_SET_WAKEUP_TIME))
		return (EFI_UNSUPPORTED);

	status = efirt_call_rt(efirt_set_wakeup_time,
	    (uint64_t)enable, (uint64_t)t, 0, 0, 0);
...
/*
 * Get the value of a UEFI variable
 * (UEFI Specification 2.10, Section 8.2).
 *
 * name is a NUL-terminated UCS-2 string.  vendor_guid identifies
 * the variable's namespace.  attrs may be NULL.  data_size is
 * in/out: on entry the buffer size, on exit the actual size.
 */
uint64_t
efi_get_variable(uint16_t *name, efi_guid_t *vendor_guid,
    uint32_t *attrs, uint64_t *data_size, void *data)
{
	uint64_t status;
	...
	status = efirt_call_rt(efirt_get_variable,
	    (uint64_t)name, (uint64_t)vendor_guid,
	    (uint64_t)attrs, (uint64_t)data_size,
	    (uint64_t)data);
...
/*
 * Enumerate UEFI variable names
 * (UEFI Specification 2.10, Section 8.2).
 *
 * name_size is in/out.  name is in/out (seed with previous or
 * empty NUL-terminated UCS-2 string).  vendor_guid is in/out.
 */
uint64_t
    ...
    efi_guid_t *vendor_guid)
{
	...
	if (!efirt_is_active())
		return (EFI_UNSUPPORTED);
	if (!efirt_is_supported(
	    EFI_RT_SUPPORTED_GET_NEXT_VARIABLE_NAME))
		return (EFI_UNSUPPORTED);

	status = efirt_call_rt(efirt_get_next_variable_name,
	    (uint64_t)name_size, (uint64_t)name,
	    (uint64_t)vendor_guid, 0, 0);
...
/*
 * Set or create a UEFI variable
 * (UEFI Specification 2.10, Section 8.2).
 *
 * name is a NUL-terminated UCS-2 string.  attrs is a bitmask of
 * EFI_VARIABLE_* flags.  Setting data_size to 0 deletes the
 * variable (when supported by firmware).
 */
uint64_t
efi_set_variable(uint16_t *name, efi_guid_t *vendor_guid,
    uint32_t attrs, uint64_t data_size, void *data)
{
	uint64_t status;
...
/*
 * Get the next high 32 bits of the monotonic count
 * (UEFI Specification 2.10, Section 8.5.2).
 */
uint64_t
efi_get_next_high_mono_count(uint32_t *high_count)
{
	uint64_t status;
	...
	status = efirt_call_rt(efirt_get_next_high_mono_count,
	    (uint64_t)high_count, 0, 0, 0, 0);
 ...
 * (UEFI Specification 2.10, Section 8.2).
 *
 * attrs specifies the variable type to query (combination of
 * EFI_VARIABLE_* flags).  The three output pointers receive the
 * maximum storage, remaining storage, and maximum individual
 * variable size for the specified type.
 */
uint64_t
efi_query_variable_info(uint32_t attrs,
    uint64_t *max_storage, uint64_t *remaining,
    uint64_t *max_size)
{
	uint64_t status;
	...
	status = efirt_call_rt(efirt_query_variable_info,
	    ...
	    (uint64_t)remaining, (uint64_t)max_size, 0);
...
/*
 * Pass capsules to firmware for processing
 * (UEFI Specification 2.10, Section 8.5.3).
 *
 * capsule_header_array is an array of pointers to
 * EFI_CAPSULE_HEADER structures.  scatter_gather_list is the
 * physical address of a scatter-gather list for capsules that
 * survive a reset (0 if not applicable).
 */
uint64_t
efi_update_capsule(void **capsule_header_array,
    uint64_t capsule_count, uint64_t scatter_gather_list)
{
	...
	if (!efirt_is_active())
		return (EFI_UNSUPPORTED);
	if (!efirt_is_supported(EFI_RT_SUPPORTED_UPDATE_CAPSULE))
		return (EFI_UNSUPPORTED);

	status = efirt_call_rt(efirt_update_capsule,
	    (uint64_t)capsule_header_array, capsule_count,
 ...
 * (UEFI Specification 2.10, Section 8.5.3).
 *
 * Returns the maximum capsule size and the required reset type
 * for the given capsule header array.
 */
uint64_t
efi_query_capsule_caps(void **capsule_header_array,
    uint64_t capsule_count, uint64_t *max_capsule_size,
    uint32_t *reset_type)
{
	uint64_t status;
	...
	if (!efirt_is_active())
		return (EFI_UNSUPPORTED);
	if (!efirt_is_supported(
	    EFI_RT_SUPPORTED_QUERY_CAPSULE_CAPS))
		return (EFI_UNSUPPORTED);

	status = efirt_call_rt(efirt_query_capsule_caps,
	    (uint64_t)capsule_header_array, capsule_count,
	    (uint64_t)max_capsule_size,
	    (uint64_t)reset_type, 0);
