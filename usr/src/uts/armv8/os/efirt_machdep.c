...
static caddr_t
efirt_pa_to_kseg(paddr_t pa)
{
	return ((caddr_t)(((paddr_t)kpm_vbase)|pa));
}
...
efirt_map_page(pte_t *l0, uint64_t pa, pte_t pte_attr)
{
	...
	if (!PTE_ISVALID(l0[l0i])) {
		...
		l1 = (pte_t *)efirt_pa_to_kseg(l0[l0i] & PTE_PFN_MASK);
	}
	...
	if (!PTE_ISVALID(l1[l1i])) {
		...
		l2 = (pte_t *)efirt_pa_to_kseg(l1[l1i] & PTE_PFN_MASK);
	}
	...
	if (!PTE_ISVALID(l2[l2i])) {
		...
		l3 = (pte_t *)efirt_pa_to_kseg(l2[l2i] & PTE_PFN_MASK);
	}
