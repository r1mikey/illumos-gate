...
#define	IOMMU_USED(rdip)	(B_FALSE)
...
static struct dev_ops rootnex_ops = {
	...
	.devo_probe		= nulldev,
	.devo_attach		= rootnex_attach,
	.devo_detach		= rootnex_detach,
	.devo_reset		= nulldev,
	.devo_cb_ops		= &rootnex_cb_ops,
	.devo_bus_ops		= &rootnex_bus_ops,
	.devo_power		= NULL,
	.devo_quiesce		= rootnex_quiesce,
};

static struct modldrv rootnex_modldrv = {
	.drv_modops		= &mod_driverops,
	.drv_linkinfo		= "ARM,sbsa root nexus",
	.drv_dev_ops		= &rootnex_ops
};

static struct modlinkage rootnex_modlinkage = {
	.ml_rev			= MODREV_1,
	.ml_linkage		= { &rootnex_modldrv, NULL }
};
...
rootnex_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	e = rootnex_dma_init();
	if (e != DDI_SUCCESS) {
		// prom_printf("rootnex_attach: rootnex_dma_init failed\n");
    ...
    off_t len, caddr_t *vaddrp)
{
	...
	case DDI_MO_MAP_HANDLE:
		break;
	default:
		// prom_printf("rootnex_map: unimplemented map op %d\n", mp->map_op);
#ifdef	DDI_MAP_DEBUG
	...
	if (mp->map_flags & DDI_MF_USER_MAPPING)  {
		// prom_printf("rootnex_map: unimplemented map type: user\n");
#ifdef	DDI_MAP_DEBUG
	...
	if (mp->map_type == DDI_MT_RNUMBER)  {
		...
		if (orp == NULL) {
			// prom_printf("rootnex_map: Out of range rnumber <%d>, device <%s>\n", rnumber, ddi_get_name(rdip));
#ifdef	DDI_MAP_DEBUG
...
rootnex_map_regspec(ddi_map_req_t *mp, caddr_t *vaddrp)
{
	...
	if (hp != NULL) {
		...
		case DDI_STORECACHING_OK_ACC:
			...
			break;
		default:
			// prom_printf("rootnex_map_regspec: unhandled hp->ah_acc.devacc_attr_dataorder\n");
			return (DDI_ME_INVAL);
		}
		ap = (ddi_acc_impl_t *)hp->ah_platform_private;
		...
		// prom_printf("rootnex_map_regspec: NULL hp, assuming HAT_STRICTORDER\n");
		hat_acc_flags = HAT_STRICTORDER;
	}
	...
	if (rp->regspec_size == 0) {
		// prom_printf("rootnex_map_regspec: zero regspec_size\n");
#ifdef  DDI_MAP_DEBUG
	...
	if (mp->map_flags & DDI_MF_DEVICE_MAPPING) {
		...
		*vaddrp = (caddr_t)((uintptr_t)mmu_btop(pbase));
...
#if 0
		prom_printf("rootnex_map_regspec: Mapping %ld pages "
		...
		if (cvaddr == NULL) {
			// prom_printf("rootnex_map_regspec: device_arena_alloc gave us NULL, returning DDI_ME_NORESOURCES\n");
...
typedef enum {
	CRS_POLARITY_ACTIVE_HIGH	= 0x00,
	CRS_POLARITY_ACTIVE_LOW		= 0x01,
	CRS_POLARITY_ACTIVE_BOTH	= 0x02
} crs_ata_irq_polarity_t;

typedef struct {
	uint32_t gsiv;
	crs_ata_irq_polarity_t polarity;
	boolean_t found;
	boolean_t edge;
	boolean_t shareable;
	boolean_t wake_capable;
} crs_cb_data_t;

static ACPI_STATUS
rootnex_crs_cb(ACPI_RESOURCE *rp, void *context)
{
	crs_cb_data_t *data = context;
	ASSERT(data != NULL);
	...
	case ACPI_RESOURCE_TYPE_EXTENDED_IRQ:
	{
		...
		if (rp->Data.ExtendedIrq.ProducerConsumer == ACPI_CONSUMER) {
			for (i = 0;
			    i < rp->Data.ExtendedIrq.InterruptCount;
			    ++i) {
				if (data->gsiv ==
				    rp->Data.ExtendedIrq.Interrupts[i]) {
					// prom_printf("rootnex_crs_cb: vec %u triggering is 0x%x - val will be %s\n", data->gsiv, rp->Data.Irq.Triggering, (rp->Data.Irq.Triggering == ACPI_EDGE_SENSITIVE) ? "B_TRUE" : "B_FALSE");
					data->edge = (rp->Data.Irq.Triggering ==
					    ACPI_EDGE_SENSITIVE) ?
					    B_TRUE : B_FALSE;
					data->shareable = rp->Data.Irq.Shareable
					    == ACPI_SHARED ? B_TRUE : B_FALSE;
					data->wake_capable =
					    rp->Data.Irq.WakeCapable ==
					    ACPI_WAKE_CAPABLE ?
					    B_TRUE : B_FALSE;
					switch (rp->Data.Irq.Polarity) {
					case ACPI_ACTIVE_HIGH:
						data->polarity =
						    CRS_POLARITY_ACTIVE_HIGH;
						break;
					case ACPI_ACTIVE_LOW:
						data->polarity =
						    CRS_POLARITY_ACTIVE_LOW;
						break;
					case ACPI_ACTIVE_BOTH:
						data->polarity =
						    CRS_POLARITY_ACTIVE_BOTH;
						break;
					default:
						break;
					}
	...
	case ACPI_RESOURCE_TYPE_IRQ:
	{
		...
		for (i = 0; i < rp->Data.Irq.InterruptCount; i++) {
			if (data->gsiv == rp->Data.Irq.Interrupts[i]) {
				data->edge = rp->Data.Irq.Triggering ==
				    ACPI_EDGE_SENSITIVE ? B_TRUE : B_FALSE;
				data->shareable =
				    rp->Data.Irq.Shareable == ACPI_SHARED ?
				    B_TRUE : B_FALSE;
				data->wake_capable = rp->Data.Irq.WakeCapable ==
				    ACPI_WAKE_CAPABLE ? B_TRUE : B_FALSE;
				switch (rp->Data.Irq.Polarity) {
				case ACPI_ACTIVE_HIGH:
					data->polarity =
					    CRS_POLARITY_ACTIVE_HIGH;
...
{
	/*
	 * Get the ACPI_HANDLE using acpica
	 * Run _CRS
	 * Iterate the results, looking for interrupts
	 * Find the IRQ that matches our GSIV
	 * Validate that the sense can be applied to this GSIV
	 * Return the sense/config/etc.
	 */
	ACPI_HANDLE rh;
	...
	data->found = B_FALSE;
	(void) AcpiWalkResources(rh, METHOD_NAME__CRS, rootnex_crs_cb, data);
...
rootnex_get_ispec(dev_info_t *rdip, int inum)
{
	...
	if (pdp == NULL || inum >= pdp->par_nintr)
		return (NULL);
...
static int
rootnex_intr_supported_types(dev_info_t *rdip,
    ddi_intr_handle_impl_t *hdlp, int *types)
{
...
{
	struct intrspec		*ispec;
	crs_cb_data_t		irqinfo;

	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_alloc_intr_fixed: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}

	if (hdlp->ih_scratch1 != 1) {
		prom_printf("rootnex_alloc_intr_fixed: called for client %s%d, interrupt index %d - count is not 1\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_EINVAL);
	}
	...
	if ((ispec = rootnex_get_ispec(rdip, hdlp->ih_inum)) == NULL) {
		prom_printf("rootnex_alloc_intr_fixed: called for client %s%d, interrupt index %d - could not get ispec\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
	...
	/*
	 * This just checks that ACPI knows about the interrupt for this device
	 */
	irqinfo.gsiv = ispec->intrspec_vec;
	if (rootnex_get_interrupt_config(rdip, &irqinfo) != DDI_SUCCESS) {
		prom_printf("rootnex_alloc_intr_fixed: called for client %s%d, interrupt index %d - could not get interrupt config\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}

	*(int *)result = hdlp->ih_scratch1;	/* always 1 */
	return (DDI_SUCCESS);
}
...
{
	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_free_intr_fixed: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
...
static int
rootnex_intr_getpri(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp, int *pri)
{
	struct intrspec		*ispec;

	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_getpri: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
	...
	if ((ispec = rootnex_get_ispec(rdip, hdlp->ih_inum)) == NULL) {
		prom_printf("rootnex_intr_getpri: called for client %s%d, interrupt index %d - could not get ispec\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
...
#if 0
	prom_printf("rootnex_intr_getpri: done for client %s%d, interrupt index %d, returning %d\n",
	    ddi_node_name(rdip), ddi_get_instance(rdip),
	    hdlp->ih_inum, *pri);
#endif
	return (DDI_SUCCESS);
 ...
 */
static int
rootnex_intr_setpri(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp, int pri)
{
	struct intrspec		*ispec;

	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_setpri: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
	...
	if (pri > LOCK_LEVEL) {
		prom_printf("rootnex_intr_setpri: called for client %s%d, interrupt index %d - pri %d is higher than LOCK_LEVEL (%d)\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum, pri, LOCK_LEVEL);
		return (DDI_FAILURE);
	}

	if ((ispec = rootnex_get_ispec(rdip, hdlp->ih_inum)) == NULL) {
		prom_printf("rootnex_intr_setpri: called for client %s%d, interrupt index %d - could not get ispec\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
...
/*
 * XXXARM: Should we look up the IRQ config here and tweak appropriately?
 */
static int
rootnex_intr_getcap(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp, int *cap)
{
	struct intrspec		*ispec;

	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_getcap: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
	...
	if ((ispec = rootnex_get_ispec(rdip, hdlp->ih_inum)) == NULL) {
		prom_printf("rootnex_intr_getcap: called for client %s%d, interrupt index %d - could not get ispec\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
	 ...
	 * DDI_INTR_FLAG_MSI64
	 */
	if (GIC_INTID_IS_SGI(hdlp->ih_inum)) {
		/* SGI is edge-triggered only */
		*cap = DDI_INTR_FLAG_EDGE;
	} else if (GIC_INTID_IS_ANY_PPI(hdlp->ih_inum) ||
	    GIC_INTID_IS_ANY_SPI(hdlp->ih_inum)) {
		/* PPI and SPI can be either level or edge-triggered */
		*cap = DDI_INTR_FLAG_EDGE | DDI_INTR_FLAG_EDGE;
	} else if (GIC_INTID_IS_LPI(hdlp->ih_inum)) {
		/* LPI is edge-triggered only */
		*cap = DDI_INTR_FLAG_EDGE;
	} else {
		prom_printf("rootnex_intr_getcap: failed for client %s%d, interrupt index %d - unknown GIC interrupt type\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
...
#if 0
	prom_printf("rootnex_intr_getcap: done for client %s%d, interrupt index %d, returning %d\n",
	    ddi_node_name(rdip), ddi_get_instance(rdip),
	    hdlp->ih_inum, *cap);
#endif
	return (DDI_SUCCESS);
...
static int
rootnex_intr_addisr(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp)
{
	struct intrspec		*ispec;

	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_addisr: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
	...
	if ((ispec = rootnex_get_ispec(rdip, hdlp->ih_inum)) == NULL) {
		prom_printf("rootnex_intr_addisr: called for client %s%d, interrupt index %d - could not get ispec\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
...
static int
rootnex_intr_remisr(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp)
{
	struct intrspec		*ispec;

	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_remisr: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
	...
	if ((ispec = rootnex_get_ispec(rdip, hdlp->ih_inum)) == NULL) {
		prom_printf("rootnex_intr_remisr: called for client %s%d, interrupt index %d - could not get ispec\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}

	ispec->intrspec_func = NULL;
	return (DDI_SUCCESS);
}

static int
rootnex_intr_nintrs(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp, int *nintrs)
{
	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_nintrs: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}

	if ((*nintrs = i_ddi_get_intx_nintrs(rdip)) == 0) {
		prom_printf("rootnex_intr_nintrs: called for client %s%d, interrupt index %d - no interrupts\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
...
#if 0
	prom_printf("rootnex_intr_nintrs: done for client %s%d, interrupt index %d, returning %d\n",
	    ddi_node_name(rdip), ddi_get_instance(rdip),
	    hdlp->ih_inum, *nintrs);
#endif
	return (DDI_SUCCESS);
...
static int
rootnex_intr_navail(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp, int *navail)
{
	return (rootnex_intr_nintrs(rdip, hdlp, navail));
}

static int
rootnex_intr_enable(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp)
{
	struct intrspec		*ispec;
	crs_cb_data_t		irqinfo;

	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_enable: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
	...
	if (hdlp->ih_dip != rdip) {
		prom_printf("rootnex_intr_enable: called for client %s%d, interrupt index %d - mismatched dips\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		if (hdlp->ih_dip != NULL) {
			prom_printf("rootnex_intr_enable: called for client %s%d, interrupt index %d - hdlp->ih_dip points to %s%d\n",
			    ddi_node_name(rdip), ddi_get_instance(rdip),
			    hdlp->ih_inum,
			    ddi_node_name(hdlp->ih_dip), ddi_get_instance(hdlp->ih_dip));
		} else {
			prom_printf("rootnex_intr_enable: called for client %s%d, interrupt index %d - hdlp->ih_dip is NULL\n",
			    ddi_node_name(rdip), ddi_get_instance(rdip),
			    hdlp->ih_inum);
		}
	...
	if ((ispec = rootnex_get_ispec(rdip, hdlp->ih_inum)) == NULL) {
		prom_printf("rootnex_intr_enable: called for client %s%d, interrupt index %d - could not get ispec\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
...
#if 0
	prom_printf("rootnex_intr_enable: called for client %s%d, interrupt index %d - vector is %d\n",
	    ddi_node_name(rdip), ddi_get_instance(rdip),
	    hdlp->ih_inum, ispec->intrspec_vec);
#endif

	irqinfo.gsiv = ispec->intrspec_vec;
	if (rootnex_get_interrupt_config(rdip, &irqinfo) != DDI_SUCCESS) {
		prom_printf("rootnex_intr_enable: called for client %s%d, "
		    "interrupt index %d - could not get interrupt config\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_FAILURE);
	}
	...
	/*
	 * We're using the older style of interrupts where the ih_vector is
	 * only authoratively assigned at enable-time.
	 */
	hdlp->ih_vector = ispec->intrspec_vec;
	((ihdl_plat_t *)hdlp->ih_private)->ip_ispecp = ispec;

	/*
	 * In ACPI we have a flat interrupt space (GSIV), so no
	...
	case 78:	/* fallthrough */
	case 79:
		/* XXXARM: check if this is necessary */
		if (irqinfo.edge == B_FALSE) {
#if 0
			prom_printf("rootnex_intr_enable: %s%d: interrupt "
			    "index %d, vector %d, set to level when it should "
			    "be edge, correcting...\n",
			    ddi_node_name(rdip), ddi_get_instance(rdip),
			    hdlp->ih_inum, ispec->intrspec_vec);
#endif
...
#if 0
	prom_printf("rootnex_intr_enable: called for client %s%d, interrupt "
	    "index %d, vector %d, %s\n",
	    ddi_node_name(rdip), ddi_get_instance(rdip),
	    hdlp->ih_inum, ispec->intrspec_vec,
	    irqinfo.edge == B_TRUE ? "edge" : "level");
#endif

	gic_config_irq(ispec->intrspec_vec, irqinfo.edge);

	/* Add the interrupt handler */
	if (!add_avintr((void *)hdlp, ispec->intrspec_pri,	/* que? */
	    hdlp->ih_cb_func, DEVI(rdip)->devi_name, hdlp->ih_vector,
	    hdlp->ih_cb_arg1, hdlp->ih_cb_arg2, NULL, rdip)) {
		prom_printf("rootnex_intr_enable: called for client %s%d, interrupt index %d, vector %d, prio %d, sense %s - could not add avintr\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum, hdlp->ih_vector, hdlp->ih_pri, irqinfo.edge == B_TRUE ? "edge" : "level");
		return (DDI_FAILURE);
	}

#if 0
	prom_printf("rootnex_intr_enable: %s%d: interrupt index %d, "
	    "vector %d, %s: enabled\n",
	    ddi_node_name(rdip), ddi_get_instance(rdip),
	    hdlp->ih_inum, ispec->intrspec_vec,
	    irqinfo.edge == B_TRUE ? "edge" : "level");
...
static int
rootnex_intr_disable(dev_info_t *rdip, ddi_intr_handle_impl_t *hdlp)
{
	if (hdlp->ih_type != DDI_INTR_TYPE_FIXED) {
		prom_printf("rootnex_intr_disable: called for client %s%d, interrupt index %d - not a fixed interrupt\n",
		    ddi_node_name(rdip), ddi_get_instance(rdip),
		    hdlp->ih_inum);
		return (DDI_ENOTSUP);
	}
	...
	/*
	 * XXXARM: should we not be doing more here?
	 */
...
{
	struct intrspec			*ispec;
	gic_intr_state_t		*gstate;
	extern kmutex_t			gic_intrs_lock;
	crs_cb_data_t			irqinfo;
	...
	case DDI_INTROP_SUPPORTED_TYPES:
		return (rootnex_intr_supported_types(
		    rdip, hdlp, (int *)result));
	case DDI_INTROP_NINTRS:
		return (rootnex_intr_nintrs(rdip, hdlp, (int *)result));
	case DDI_INTROP_ALLOC:
		return (rootnex_alloc_intr_fixed(rdip, hdlp, result));
	case DDI_INTROP_GETPRI:
		return (rootnex_intr_getpri(rdip, hdlp, (int *)result));
	case DDI_INTROP_SETPRI:
		return (rootnex_intr_setpri(rdip, hdlp, *((int *)result)));
	case DDI_INTROP_ADDISR:
		return (rootnex_intr_addisr(rdip, hdlp));
	case DDI_INTROP_ENABLE:
		return (rootnex_intr_enable(rdip, hdlp));
	case DDI_INTROP_DISABLE:
		return (rootnex_intr_disable(rdip, hdlp));
	case DDI_INTROP_REMISR:
		return (rootnex_intr_remisr(rdip, hdlp));
	case DDI_INTROP_FREE:
		return (rootnex_free_intr_fixed(rdip, hdlp));
	case DDI_INTROP_GETCAP:
		return (rootnex_intr_getcap(rdip, hdlp, (int *)result));
	case DDI_INTROP_NAVAIL:
		return (rootnex_intr_navail(rdip, hdlp, (int *)result));

	/*
	 * Unsupported interrupt operations
	 */

	/* useful, but never called! we could use this */
	case DDI_INTROP_GETTARGET:	/* fallthrough */
	/* called in common/io/mac/mac_datapath_setup.c - we should have this */
...
#if 0
		prom_printf("rootnex_intr_ops: %s%d: unsupported operation %d "
		    "for client %s%d, interrupt index %d\n",
		    ddi_node_name(pdip), ddi_get_instance(pdip), intr_op,
		    ddi_node_name(rdip), ddi_get_instance(rdip), hdlp->ih_inum);
#endif
		return (DDI_ENOTSUP);
	default:
#if 0
		prom_printf("rootnex_intr_ops: %s%d: unknown operation %d for "
		    "client %s%d, interrupt index %d\n",
		    ddi_node_name(pdip), ddi_get_instance(pdip), intr_op,
		    ddi_node_name(rdip), ddi_get_instance(rdip), hdlp->ih_inum);
#endif
		break;
