...
#if defined(__x86)
		/* XXXPCI: Why does this check here? */
		if (psm_intr_ops == NULL)
			return (DDI_FAILURE);
#endif
    ...
    ddi_intr_handle_impl_t *hdlp, uint32_t inum)
{
	...
	hdlp->ih_vector = GIC_VEC_TO_IRQ(
	    ((ihdl_plat_t *)hdlp->ih_private)->ip_gic_cfg, hdlp->ih_vector);
	DDI_INTR_NEXDBG((CE_CONT, "pci_enable_intr: priority=%x irq=%x\n",
    ...
    ddi_intr_handle_impl_t *hdlp, uint32_t inum)
{
	...
	/*
	 * XXXARM: The handle was already translated when the interrupt was
	 * enabled.
	 *
