 ...
 *
 * Common currency between the cpuidle framework and firmware-specific
 * drivers (acpicpu for ACPI _LPI, fdtcpu for DT idle-states).  The
 * entry method is encoded as an entry type and a PSCI power_state
...
typedef struct lpi_state {
	...
	uint32_t	ls_wake_latency;	/* Worst-case wake latency (us) */
	uint32_t	ls_ctx_loss_flags;	/* LPI_CTX_LOSS_* flags */
	uint32_t	ls_psci_state;		/* PSCI CPU_SUSPEND power_state */
	...
	uint64_t	ls_entries;		/* Times this state was entered */
 ...
 *
 * Called from firmware-specific drivers (acpicpu, fdtcpu) after parsing
 * idle state descriptors.  If no WFI entry exists in the array, one is
 * added.  The array is then sorted by min_residency ascending so that
 * lpi_select_state() can walk from deepest to shallowest reliably.
 * Passing NULL/0 is valid and registers the CPU with WFI only.
 *
 * Takes ownership of the caller-allocated states array and stores the
 * (possibly augmented) array in the per-CPU machcpu structure.
 *
 * On failure the states array is freed and the caller need not clean up.
 *
