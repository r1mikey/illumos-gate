 ...
 * handles to consumers such as CPPC, RASF, PDTT and MPST, as well as
 * to the PCC OperationRegion handler.
 *
...
/*
 * Platform notification callback.  Invoked when the platform raises
 * a notification on the channel (Type 4 responder subspaces, and the
 * deprecated Type 1/2 platform notification mechanism).  Runs in
 * interrupt context; must not block.
 */
typedef void (*pcc_notify_fn_t)(void *arg);
...
/* Type 3-4 extended PCC header (16 bytes). */
 ...
 * bring up usable channels.  Called once from acpipcc _init, which
 * runs after the acpica module (an ELF dependency) has initialised.
 * Consumers must not call this; pcc_chan_get works once acpipcc has
 * loaded.
...
/*
 * Tear down PCC: remove the OperationRegion handler, release
 * interrupts, unmap registers and shared memory, and free channels.
 * The module's _fini refuses unload (channels may be in use), so
 * this is not called in normal operation; it exists to reverse
 * pcc_init for completeness.
 */
void pcc_teardown(void);

/* PCC OperationRegion handler registration */
int pcc_opregion_register(void);
void pcc_opregion_unregister(void);

/*
 * Look up a channel by firmware subspace ID (the PCCT subtable index,
 * also used as the PCC OperationRegion Address and in CPPC _CPC
 * packages).  This is NOT the channel's index in an array.
 *
 ...
 *
 * The caller must hold the channel mutex.  Returns DDI_SUCCESS,
 * DDI_FAILURE (transport or protocol error), or DDI_ENOTSUP (the
 * channel type cannot send; e.g. Type 4 responder subspaces).
 */
 ...
 * Returns DDI_SUCCESS, DDI_FAILURE, DDI_ENOTSUP or EBUSY.
 */
int pcc_chan_send_nowait(pcc_chan_t *pc, uint32_t cmd, uint32_t payload_len);

/*
 * Register a platform notification callback for the channel.  Used by
 * Type 4 responder subspaces; Types 1 and 2 support the deprecated
 * platform notification mechanism.  Acquires the channel mutex
 * itself.  Returns DDI_SUCCESS, or DDI_ENOTSUP if the channel cannot
 * deliver notifications (no interrupt, or polling-only mode).
 */
int pcc_chan_register_notify(pcc_chan_t *pc, pcc_notify_fn_t fn, void *arg);
