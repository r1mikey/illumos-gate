...
typedef void (*pgo_config_irq_t)(void *ctx, uint32_t irq, bool is_edge);
...
typedef uint32_t (*pgo_acknowledge_t)(void *ctx);
typedef uint32_t (*pgo_ack_to_vector_t)(void *ctx, uint32_t ack);
typedef void (*pgo_eoi_t)(void *ctx, uint32_t ack);
typedef void (*pgo_deactivate_t)(void *ctx, uint32_t ack);

typedef int (*pgo_vector_is_special_t)(void *ctx, uint32_t intid);
...
typedef struct {
	...
	pgo_vector_is_special_t	pgo_vector_is_special;
} psm_gic_ops_t;

typedef struct psm_gic_t;
