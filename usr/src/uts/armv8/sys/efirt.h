...
/*
 * UEFI Runtime Services types a declarations.
 */
...
/*
 * EFI variable attributes (UEFI Specification 2.10, Section 8.2).
 */
...
#define	EFI_VARIABLE_TIME_BASED_AUTHENTICATED	0x00000020
#define	EFI_VARIABLE_APPEND_WRITE		0x00000040

extern void efi_reset_system(EFI_RESET_TYPE, uint64_t, uint64_t,
    void *);
extern uint64_t efi_get_time(EFI_TIME *, EFI_TIME_CAPABILITIES *);
extern uint64_t efi_set_time(EFI_TIME *);
extern uint64_t efi_get_wakeup_time(uint8_t *, uint8_t *,
    EFI_TIME *);
extern uint64_t efi_set_wakeup_time(uint8_t, EFI_TIME *);
extern uint64_t efi_get_variable(uint16_t *, efi_guid_t *,
    uint32_t *, uint64_t *, void *);
extern uint64_t efi_get_next_variable_name(uint64_t *, uint16_t *,
    efi_guid_t *);
extern uint64_t efi_set_variable(uint16_t *, efi_guid_t *,
    uint32_t, uint64_t, void *);
extern uint64_t efi_get_next_high_mono_count(uint32_t *);
extern uint64_t efi_query_variable_info(uint32_t, uint64_t *,
    uint64_t *, uint64_t *);
extern uint64_t efi_update_capsule(void **, uint64_t, uint64_t);
extern uint64_t efi_query_capsule_caps(void **, uint64_t,
    uint64_t *, uint32_t *);
