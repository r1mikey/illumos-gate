...
struct	psm_ops {
	...
	int	(*psm_intr_enter)(int ipl, psm_intr_cookie_t *cookiep,
	    psm_intr_vector_t *vectorp);
	void	(*psm_intr_exit)(int ipl, psm_intr_cookie_t cookie);
	void	(*psm_setspl)(int ipl);
	int	(*psm_config_irq)(uint32_t irqno, int flags);
	...
	void	(*psm_send_ipi)(cpuset_t cpus, int ipl);
...
/*
 * version
 * 0xAAvm where v = (version no. - 1) and m = (minor no. + 1)
 * i.e. psmi 1.0 has v=0 and m=1, psmi 1.1 has v=0 and m=2
 * also, 0xAA in the high byte is the signature of the psmi
 */
#define	PSM_INFO_VER01		0xAA01
#define	PSM_INFO_VER01_0	(PSM_INFO_VER01)
#define	PSM_INFO_VER01_X	(PSM_INFO_VER01 & 0xFFF0)	/* ver 1.X */
...
#define	PSM_SPURIOUS_INTERRUPT	-1
