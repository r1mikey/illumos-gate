...
gicv2_cpu_init_raw(gicv2_conf_t *gc, cpu_t *cp)
{
	...
	if (cp->cpu_id == 0) {
		...
		if ((gicc_read(gc, GICC_PMR) & 0xf) == 0) {
			/* XXXARM: these should not be globals */
		...
		for (int i = 0; i < 8; ++i) {
			...
			gc->gc_priority[i] = gicd_read(gc, GICD_IPRIORITYRn(i));
		}
	} else {
		/*
		 ...
		 */
		for (int i = 0; i < 8; ++i) {
			gicd_write(gc, GICD_IPRIORITYRn(i), gc->gc_priority[i]);
		}
...
gicv2_init(psm_gic_t *pg)
{
	 ...
	 */
	conf.gc_icfgr1 = gicd_read(gc, GICD_ICFGRn(1));
	 ...
	 */
	gicv2_cpu_init_raw(CPU);	/* need to pass the GIC config */
...
armbsa_gicv2_init(psm_gic_t *pg)
{
	...
	pg->pg_ops.pgo_cpu_init = gicv2_cpu_init;
	pg->pg_ops.pgo_config_irq = gicv2_config_irq;
	pg->pg_ops.pgo_addspl = gicv2_addspl;
	pg->pg_ops.pgo_delspl = gicv2_delspl;
	pg->pg_ops.pgo_setlvl = gicv2_setlvl;
	pg->pg_ops.pgo_setlvlx = gicv2_setlvlx;
	pg->pg_ops.pgo_send_ipi = gicv2_send_ipi;
	pg->pg_ops.pgo_acknowledge = gicv2_acknowledge;
	pg->pg_ops.pgo_ack_to_vector = gicv2_ack_to_vector;
	pg->pg_ops.pgo_eoi = gicv2_eoi;
	pg->pg_ops.pgo_deactivate = gicv2_deactivate;
	pg->pg_ops.pgo_vector_is_special = NULL;
	return (PSM_SUCCESS);
}
