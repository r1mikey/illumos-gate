 ...
 * drivers (acpicpu, fdtcpu) can attach to the children.
 */
 ...
 *
 * For cpu nodes, reg contains the MPIDR value.  The parent's
 * #address-cells determines the width; #size-cells is 0.
 */
static int
cpunex_initchild(dev_info_t *parent, dev_info_t *child)
{
	char *devtype;
	int addr_cells;
	int *reg;
	...
	if (ddi_prop_lookup_string(DDI_DEV_T_ANY, child,
	    DDI_PROP_DONTPASS, OBP_DEVICETYPE, &devtype) !=
	    DDI_PROP_SUCCESS || strcmp(devtype, "cpu") != 0) {
		if (devtype != NULL) {
	...
	addr_cells = ddi_prop_get_int(DDI_DEV_T_ANY, parent,
	    DDI_PROP_DONTPASS, OBP_ADDRESS_CELLS, 2);

	if (addr_cells == 2 && reg_len >= 2) {
		val = ((uint64_t)(uint32_t)reg[0] << 32) |
		    (uint32_t)reg[1];
    ...
    ddi_ctl_enum_t op, void *arg, void *result)
{
	...
	case DDI_CTLOPS_INITCHILD:
		return (cpunex_initchild(dip, (dev_info_t *)arg));
