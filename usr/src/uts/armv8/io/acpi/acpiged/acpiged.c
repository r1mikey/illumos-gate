 ...
 * 4. Each ISR dispatches to a taskq that evaluates _EVT(gsiv), where
 *    the AML performs device-specific dispatch (e.g. Notify(PWRB, 0x80)).
 *
 ...
 *
 * See ACPI Specification §5.6.4.2 "Generic Event Device".
 */
...
/*
 * Taskq callback - evaluate _EVT(gsiv) under the ACPICA interpreter.
 */
static void
acpiged_evt_task(void *arg)
{
	...
	if (ACPI_FAILURE(AcpiEvaluateObject(sp->agd_hdl, "_EVT",
	    &args, NULL))) {
		dev_err(sp->agd_dip, CE_WARN,
		    "!acpiged: _EVT(0x%x) evaluation failed", ip->agi_gsiv);
	}
}
...
acpiged_intr(caddr_t arg1, caddr_t arg2 __unused)
{
	...
	if (taskq_dispatch(ip->agi_sp->agd_taskq, acpiged_evt_task,
		...
		dev_err(ip->agi_sp->agd_dip, CE_WARN,
		    "!acpiged: failed to dispatch _EVT for GSIV %u",
		    ip->agi_gsiv);
	}
...
acpiged_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 * as _EVT takes the ACPICA interpreter lock and events are serialised.
	 */
