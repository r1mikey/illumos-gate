...
/*
 * XXXARM: Significant work is needed around CPU bringup and associating the
 * CPUs to these devices in a sensible way. At the moment I'm simply hedging
 * on the idea that the MPIDR will be enough/
 */
...
acpidev_cpu_parse_MADT(ACPI_SUBTABLE_HEADER *ap, void *context)
{
	...
	case ACPI_MADT_TYPE_GENERIC_INTERRUPT:
		...
		if (gicc->Flags & ACPI_MADT_ENABLED) {
			...
			acpidev_cpu_map[*cntp].apic_id = gicc->ArmMpidr;
			(*cntp)++;
		}
		break;
#endif	/* __aarch64__ */
...
static ACPI_STATUS
acpidev_cpu_get_apicid(proc_id_t procid, apic_id_t *apicidp)
{
	uint32_t i;
