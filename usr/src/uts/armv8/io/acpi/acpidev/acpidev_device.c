...
acpidev_device_resource_process(acpidev_walk_info_t *infop)
{
	...
	if (ACPI_SUCCESS(rc) && !ddi_prop_exists(DDI_DEV_T_ANY, infop->awi_dip,
...
acpidev_device_init(acpidev_walk_info_t *infop)
{
	...
	if (infop->awi_info->Valid & ACPI_VALID_HID &&
	    infop->awi_info->HardwareId.Length >= 1) {
		nhid = 1;
	}
	...
	entries = nhid + ncid + ncls + nstatic;

	compat = kmem_zalloc(sizeof (char *) * entries, KM_SLEEP);

	i = 0;

	/* Add the _HID, if present */
	if (nhid != 0) {
		VERIFY3U(nhid, ==, 1);
		compat[i] =
		    ddi_strdup(infop->awi_info->HardwareId.String, KM_SLEEP);
		i++;
	}
