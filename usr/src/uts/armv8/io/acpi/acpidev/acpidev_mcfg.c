...
create_root_complex(acpi_root_complex_t *arc)
{
	...
	/*
	 * Set MSI and DMA coherency properties from IORT.
	 */
	{
		...
		if (irc != NULL) {
			...
			if (irc->air_simple) {
				...
				its_dip = acpidev_gic_find_its_by_trid(
				    irc->air_its_trid);
				if (its_dip != NULL) {
					(void) ndi_prop_update_int(
					    DDI_DEV_T_NONE, rdip,
					    "msi-parent",
					    ddi_get_nodeid(its_dip));
				}
				...
				for (i = 0; i < irc->air_nmaps; i++) {
					...
					msi_map[i * 4 + 1] = (its_dip != NULL) ?
					    ddi_get_nodeid(its_dip) : 0;
					msi_map[i * 4 + 2] =
