...
acpidev_boot_probe(int type)
{
	...
	if (type == 0 && acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
		...
		if (acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
			...
			if (&plat_pcierc_set_takeover) {
				plat_pcierc_set_takeover(acpidev_pcie_osc);
				cmn_err(CE_CONT, "?acpidev: plat_pcierc_set_takeover called\n");
			} else {
				cmn_err(CE_CONT, "?acpidev: plat_pcierc_set_takeover is NULL\n");
			}
		}
