...
/*
 * Firmware-specific device enumerator for ACPI systems.
 *
 * Uses static ACPI table lookup and dynamic ACPI namespace traversal to build
 * standard (FDT-compatible) device tree nodes that illumos can consume.
 *
 * Examples of static tables include:
 *   * Building a system interrupt controller node from the MADT
 *   * Building an architected timer node from the GTMR
 *   * Building PCIe root complexes from the MCFG (and DSDT)
 *
 * Examples of namespace traversal include:
 *   * Processors
 *   * UARTs
 *   * ACPI-defined devices: generic event device, power button
 *
 * We do not (yet?) create device nodes from SMBIOS tables, as modern systems
 * include all necessary devices in the DSDT and/or SSDTs.
 *
 * Devices created from firmware records are placed in the '/devices/fw' path,
 * the same as x86 systems.
 */
...
acpidev_class_list_init(uint64_t *fp)
{
	 ...
	 * Currently only ACPI scope objects under ACPI root node, such as _SB,
	 * _TZ etc, need to be handled, so only register the scope class
...
acpidev_create_root_node(void)
{
	...
	if (rv == NDI_SUCCESS) {
		rv = ddi_prop_update_int(DDI_DEV_T_NONE, dip,
		    OBP_ADDRESS_CELLS, ACPIDEV_ROOTNEX_ADDRESS_CELLS);
	}
	if (rv == NDI_SUCCESS) {
		rv = ddi_prop_update_int(DDI_DEV_T_NONE, dip,
		    OBP_SIZE_CELLS, ACPIDEV_ROOTNEX_SIZE_CELLS);
	}
	if (rv != DDI_SUCCESS) {
		ACPIDEV_DEBUG(CE_WARN,
...
acpidev_boot_probe(int type)
{
	...
	/* Probe statically defined ACPI devices */
	if (type == 0 && acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
		dev_info_t *gdip = NULL;

		if (ACPI_FAILURE(acpidev_set_up_root_node())) {
			cmn_err(CE_WARN, "!acpidev: failed to set up "
			    "the DDI root node.");
			acpidev_status = ACPIDEV_STATUS_FAILED;
		}

		/*
		 * Evaluate \_SB._OSC to declare platform-wide OS
		 * capabilities to the firmware.  Non-fatal if _OSC
		 * is absent or evaluation fails.
		 */
		if (acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
			acpidev_osc_init();

			acpidev_pcie_osc_register();
		}

		if (acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
			if (ACPI_FAILURE(acpidev_create_gic_node(&gdip))) {
				cmn_err(CE_WARN, "!acpidev: failed to "
				    "create system GIC device from MADT.");
				acpidev_status = ACPIDEV_STATUS_FAILED;
			}
		}
		...
		if (acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
			...
			if (ACPI_FAILURE(acpidev_set_system_pic(gdip))) {
				cmn_err(CE_WARN, "!acpidev: failed to "
				    "set the root interrupt parent.");
				acpidev_status = ACPIDEV_STATUS_FAILED;
			}
		}

		if (acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
			if (ACPI_FAILURE(acpidev_create_arm_gtmr_node())) {
				cmn_err(CE_WARN, "!acpidev: failed to "
				    "create generic timer device from GTDT.");
				acpidev_status = ACPIDEV_STATUS_FAILED;
			}
		}

		if (acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
			if (ACPI_FAILURE(acpidev_iort_init())) {
				cmn_err(CE_WARN, "!acpidev: failed to "
				    "parse IORT table.");
				/*
				 * Non-fatal: MSI properties will be
				 * missing but INTx still works.
				 */
			}
		}

		if (acpidev_status == ACPIDEV_STATUS_INITIALIZED) {
			if (ACPI_FAILURE(acpidev_process_mcfg(gdip))) {
				cmn_err(CE_WARN, "!acpidev: failed to "
				    "process PCI devices.");
				acpidev_status = ACPIDEV_STATUS_FAILED;
			}
	...
	/*
	 * If the static probe sequence failed, clean up any
	 * IORT and GIC registry data that was allocated before
	 * the failure.  Both functions are safe to call when
	 * their respective init was never reached.
	 */
	if (type == 0 && acpidev_status == ACPIDEV_STATUS_FAILED) {
		acpidev_iort_fini();
		acpidev_gic_fini();
	}
...
acpidev_filter_default(acpidev_walk_info_t *infop, ACPI_HANDLE hdl __unused,
