...
acpidev_boot_probe_device(acpidev_op_type_t op_type)
{
	...
	if (op_type == ACPIDEV_OP_BOOT_REPROBE) {
		/*
		 * ndi_hold_devi(dip);
		 * i_ddi_attach_node_hierarchy(dip);
		 * ndi_rele_devi(dip);
		 */
		/* find walk info with the known consumer bit set */
		/* for each of those, i_ddi_attach_node_hierarchy */
		/* XXX: online the leaf nodes here? via i_ddi_attach_node_hierarchy, only in the reprobe case */
	}
#endif	/* __aarch64__ */
