 ...
 * under the ACPI root object, such as _SB_ and _TZ_ etc.
...
static acpidev_filter_rule_t acpidev_scope_filters[] = {
	...
	/*
	 * i86pc handles the _PR_ scope, which used to be where ACPI placed
	 * processor objects.
	 *
	 * DEN0044 1.0, §8.5.1 global Methods and objects states:
	 *   Platforms must not define processors using the global
	 *   \_PR (Processors) namespace.
	 */
...
acpidev_scope_init(acpidev_walk_info_t *infop)
{
	...
	if (ddi_prop_update_int(DDI_DEV_T_NONE, infop->awi_dip,
	    OBP_ADDRESS_CELLS, ACPIDEV_ROOTNEX_ADDRESS_CELLS) != DDI_SUCCESS) {
		return (AE_ERROR);
	}
	if (ddi_prop_update_int(DDI_DEV_T_NONE, infop->awi_dip,
	    OBP_SIZE_CELLS, ACPIDEV_ROOTNEX_SIZE_CELLS) != DDI_SUCCESS) {
		return (AE_ERROR);
	}
