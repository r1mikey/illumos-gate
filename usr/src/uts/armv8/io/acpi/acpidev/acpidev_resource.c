...
ACPI_STATUS
acpidev_resource_insert_irq(acpidev_resource_handle_t rhdl, int irq, int flags)
{
	ASSERT(rhdl != NULL);
	...
	rhdl->acpidev_irqf[rhdl->acpidev_irq_count] = flags;
...
acpidev_resource_address64(acpidev_resource_handle_t rhdl,
    ACPI_RESOURCE_ADDRESS64 *addrp, boolean_t is_res64)
{
	...
	case ACPI_IO_RANGE:
		...
		/* Generate 'ranges' for producer. */
			...
			range.parent_low = paddr & 0xFFFFFFFF;
			/*
			 * If it's memory on the parent side we record whether
			 * 64-bit is needed or not.
			 */
			if (addrp->Info.Io.Translation &&
			    range.parent_mid != 0) {
				range.parent_hi |= ACPIDEV_REG_MEM_64BIT;
			}
			range.size_hi = addrp->Address.AddressLength >> 32;
			range.size_low = addrp->Address.AddressLength &
			    0xFFFFFFFF;
...
acpidev_resource_walk_producer(ACPI_RESOURCE *rscp, void *ctxp)
{
	...
	case ACPI_RESOURCE_TYPE_ADDRESS64:
	{
		...
		boolean_t is_res64 =
		    (rscp->Type == ACPI_RESOURCE_TYPE_ADDRESS64) ?
		    B_TRUE : B_FALSE;
...
acpidev_resource_walk_consumer(ACPI_RESOURCE *rscp, void *ctxp)
{
	...
	case ACPI_RESOURCE_TYPE_EXTENDED_IRQ:
	{
		...
		if (rscp->Data.ExtendedIrq.Triggering == ACPI_EDGE_SENSITIVE) {
			f |= AGIF_EDGE;
			if (rscp->Data.ExtendedIrq.Polarity ==
			    ACPI_ACTIVE_LOW) {
				f |= AGIF_FALLING;
			}
		} else {
		...
		for (i = 0; i < rscp->Data.ExtendedIrq.InterruptCount; i++) {
			if (ACPI_SUCCESS(acpidev_resource_insert_irq(rhdl,
			    rscp->Data.ExtendedIrq.Interrupts[i], f))) {
				continue;
			}
	...
	case ACPI_RESOURCE_TYPE_IRQ:
	{
		...
		if (rscp->Data.Irq.Triggering == ACPI_EDGE_SENSITIVE) {
			f |= AGIF_EDGE;
			if (rscp->Data.Irq.Polarity == ACPI_ACTIVE_LOW) {
				f |= AGIF_FALLING;
			}
		} else {
			if (rscp->Data.Irq.Polarity == ACPI_ACTIVE_LOW) {
...
static int
acpidev_resource_create_interrupts(acpidev_walk_info_t *infop,
    acpidev_resource_handle_t rhdl)
{
	...
	ngi = rhdl->acpidev_irq_count;
	alloc_sz = ngi * sizeof (struct acpidev_gic_interrupt);
	gi = kmem_zalloc(alloc_sz, KM_SLEEP);
	ASSERT3P(gi, !=, NULL);

	for (i = 0; i < rhdl->acpidev_irq_count; ++i) {
		gi[i].gi_gsiv = rhdl->acpidev_irqp[i];
		gi[i].gi_flags = rhdl->acpidev_irqf[i];
	}

	if (ACPI_FAILURE(
	    acpidev_set_gsiv_interrupts(infop->awi_dip, gi, ngi))) {
		rv = NDI_FAILURE;
	} else {
		rv = NDI_SUCCESS;
	}

	kmem_free(gi, alloc_sz);
	return (rv);
}
...
acpidev_resource_process(acpidev_walk_info_t *infop, boolean_t consumer)
{
	...
	if (consumer) {
		...
		if (rhdl->acpidev_irq_count > 0 &&
		    acpidev_resource_create_interrupts(infop, rhdl) !=
		    NDI_SUCCESS) {
			ACPIDEV_DEBUG(CE_WARN, "!acpidev: failed to set "
