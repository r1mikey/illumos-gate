    ...
    ACPI_RESOURCE_ADDRESS64 *addrp, boolean_t is_res64)
{
	...
	case ACPI_IO_RANGE:
		...
		/* Generate 'ranges' for producer. */
			 ...
			 */
			if (addrp->Info.Io.Translation && range.parent_mid != 0)
				range.parent_hi |= ACPIDEV_RES_MEM_64BIT;
			range.size_hi = addrp->Address.AddressLength >> 32;
			range.size_low = addrp->Address.AddressLength &
			    0xFFFFFFFF;
			rc = acpidev_resource_insert_range(rhdl, &range);
    ...
    acpidev_resource_handle_t rhdl)
{
	...
	if (ACPI_FAILURE(acpidev_set_gsiv_interrupts(infop->awi_dip, gi, ngi)))
		rv = NDI_FAILURE;
	else
		rv = NDI_SUCCESS;
