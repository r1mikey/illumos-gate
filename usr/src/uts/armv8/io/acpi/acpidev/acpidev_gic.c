...
static struct its_entry *its_list = NULL;
...
ACPI_STATUS
acpidev_serialize_interrupt(struct acpidev_gic_interrupt *g, uint32_t *cells)
{
	uint32_t i;
...
acpidev_set_gsiv_interrupts(dev_info_t *rdip,
    struct acpidev_gic_interrupt *gi, size_t ngi)
{
	size_t gii;
	size_t i;
	size_t ncells;
...
acpidev_set_mmio_regs(dev_info_t *rdip, struct regspec *rs, size_t nrs)
{
	...
	for (rsi = 0; rsi < nrs; ++rsi) {
		...
		switch (ac) {
		case 3:
			cells[i++] = (r->regspec_bustype >> 32) & 0xffffffff;
			/* fallthrough */
		case 2:
...
gic_setup_v2_device(dev_info_t *rdip, struct madt_gic *mg, bool has_children)
{
	...
	for (gicc = mg->mg_gicc; gicc != NULL; gicc = gicc->next) {
		...
		if (vgic_intid_flags == 0)
			vgic_intid_flags = gicc->gicc->Flags & (1U << 2);

		if ((gicc->gicc->Flags & (1U << 2)) != vgic_intid_flags)
	...
	if (gich_addr != 0 && gicv_addr != 0 && vgic_intid != 0) {
		...
		vgic_agi.gi_flags = (vgic_intid_flags & (1U << 2)) ?
...
gic_setup_v2_node(dev_info_t *dip, struct madt_gic *mg, dev_info_t **xdipp)
{
	...
	if ((rv = gic_setup_v2_device(rdip, mg, mg->mg_gv2m != NULL)) !=
	    DDI_SUCCESS)
		goto out;
	...
	for (gv2m = mg->mg_gv2m; gv2m != NULL; gv2m = gv2m->next) {
		...
		if ((rv = gic_setup_v2m_child(gdip, gv2m->gv2m)) != DDI_SUCCESS)
			goto out;
...
gic_setup_v3_device(dev_info_t *rdip, struct madt_gic *mg, bool has_children)
{
	...
	/*
	 * GIC Redistributor region size for CPU-bound redistributors.
	 *
	...
	for (gicc = mg->mg_gicc; gicc != NULL; gicc = gicc->next) {
		...
		if (vgic_maintenance_interrupt != 0 &&
			...
			panic("Inconsitent VGIC maintenance interrupt in MADT. "
	 ...
	 * power domain we will have redistrubutors exposed via the GICC
	 * structures.
	 *
	...
	if ((rv = ndi_prop_update_int(DDI_DEV_T_NONE, rdip,
	    OBP_INTERRUPT_CELLS, ACPI_GIC_INTERRUPT_CELLS)) != DDI_PROP_SUCCESS)
		goto out;
...
gic_setup_v3_child(dev_info_t *rdip, ACPI_MADT_GENERIC_TRANSLATOR *gits)
{
	...
	/* 128k - seems to add another 64k frame in 4.1 */
	gits_size = 0x20000;
...
gic_setup_v3_node(dev_info_t *dip, struct madt_gic *mg, dev_info_t **xdipp)
{
	...
	if ((rv = gic_setup_v3_device(rdip, mg, mg->mg_gits != NULL)) !=
	    DDI_SUCCESS)
		goto out;
...
acpidev_gic_setup_device_node(dev_info_t **xdipp)
{
	...
	if ((st = AcpiGetTable(ACPI_SIG_MADT, 1,
		...
		panic("Unable to get the MADT: 0x%u", st);
	...
	VERIFY3U(mg.mg_gicd->gicd->Version, >=, 2);
	VERIFY3U(mg.mg_gicd->gicd->Version, <=, 4);
...
acpidev_gic_find_its_by_trid(uint32_t trid)
{
	...
	for (ite = its_list; ite != NULL; ite = ite->ite_next) {
...
{
	if (its_list == NULL || its_list->ite_next != NULL)
		return (B_FALSE);

	*tridp = its_list->ite_trid;
	return (B_TRUE);
}
