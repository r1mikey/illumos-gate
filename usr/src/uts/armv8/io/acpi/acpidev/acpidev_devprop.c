 ...
 * and new names when a substritution happens.
 */
static char *
    ...
    const ACPI_OBJECT *name, const ACPI_OBJECT *val)
{
	...
	if (rc == DDI_SUCCESS && key_name != name->String.Pointer) {
		ddi_prop_update_byte_array(dev, infop->awi_dip,
    ...
    const ACPI_OBJECT *name, const ACPI_OBJECT *val)
{
	...
	/* XXXARM: We should handle reference here */
	default:
		/* XXXARM: warn? */
		break;
	}
	...
	for (i = 0; i < dsd->Package.Count; i += 2) {
		...
		for (i = 0; i < dsd_pkg->Package.Count; i ++) {
			...
			case ACPI_TYPE_PACKAGE:
				...
				break;
			/* XXXARM: We should handle reference objects here */
			default:
				/* XXXARM: warn? */
				break;
			}
...
acpidev_devprop_process_dsd(acpidev_walk_info_t *infop)
{
	...
	if (nnode->Type == ACPI_TYPE_PACKAGE) {
		...
		dsd.Pointer = NULL;
		st = AcpiEvaluateObjectTyped(target, NULL, NULL,
