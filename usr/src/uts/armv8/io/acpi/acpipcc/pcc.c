...
/*
 * ACPI PCC (Platform Communications Channel) common code and public interface.
 *
...
static pcc_chan_t *pcc_channels;	/* channel array */
...
/*
 * Subspace type dispatch table.  Maps PCCT subtable type to the minimum
 * subtable length and the type's ops vector.
 */
static const struct {
...
static uint_t pcc_irq_handler(caddr_t arg1, caddr_t arg2);
 ...
 *
 * The chan_id is the firmware subspace ID (PCCT subtable index or CPC package
 * value), NOT the internal array index.
 *
 * Returns NULL if PCC is not initialised, the ID is unknown, or the channel
 * is not usable.
 *
 * On success, increments pc_refcnt under pc_lock.  The caller must call
 * pcc_chan_put when done.
 */
pcc_chan_t *
...
/*
 * Release a channel handle obtained from pcc_chan_get.
 *
 * Decrements the consumer reference count.  The caller must not hold the
 * channel mutex.
 */
void
pcc_chan_put(pcc_chan_t *pc)
{
	mutex_enter(&pc->pc_lock);
...
/*
 * Channel shared-memory accessors.
 *
 * The offset argument is relative to the start of the communication space,
 * so after the type-specific header.
 *
 * Callers must hold pc_lock.
 *
 * Returns DDI_FAILURE if out of bounds.
 */
...
static uint_t
pcc_irq_handler(caddr_t arg1, caddr_t arg2 __unused)
{
 ...
 *
 * The caller must hold pc_lock and must have written the payload into the
 * communication space via the pcc_chan_write* accessors before calling.
 *
 * The payload_len argument is the payload byte count; for Types 1-2 it is
 * ignored (the platform uses the PCCT Length).
 *
 * Returns DDI_SUCCESS, DDI_FAILURE (transport/protocol or platform-reported
 * error, or channel previously marked failed after a completion timeout).
 */
int
pcc_chan_send(pcc_chan_t *pc, uint32_t cmd, uint32_t payload_len)
{
	...
	ret = pc->pc_ops->pto_prepare_send(pc, cmd, payload_len);
	if (ret != DDI_SUCCESS) {
		return (ret);
	}
