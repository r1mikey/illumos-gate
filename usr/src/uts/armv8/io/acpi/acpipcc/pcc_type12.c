...
pcc_type12_parse(void *subtable, pcc_chan_t *pc)
{
	...
	if (ackg != NULL) {
		...
		/*
		 * Type 2's Platform Interrupt Ack register is required
		 * only for level-triggered interrupts: without it a
		 * level-triggered interrupt cannot be cleared (ACPI 6.6
		 * s14.5 step 9).  An edge-triggered interrupt needs no
		 * ack, so the register may be omitted (Table 14.7).  The
		 * ack masks ride along in the raw fields above.
		 */
		if (pc->pc_gsiv != 0 && ackg->Address == 0 &&
		    (pc->pc_irq_flags & ACPI_PCCT_INTERRUPT_MODE) == 0) {
			cmn_err(CE_WARN, "acpipcc: subspace %u: level-"
			    "triggered platform interrupt without ack "
...
pcc_type12_prepare_send(pcc_chan_t *pc, uint32_t cmd,
	uint32_t __unused payload_len, boolean_t trylock)
{
	 ...
	 * off, which satisfies Table 14.10's "must be cleared" rule.
	 *
	...
	*cmdfield = (uint16_t)cmd |
	 ...
	 * (step 4, rung by common code after this returns, or below
	 * on the panic path).
	 */
	membar_producer();
	...
	if (trylock) {
		/*
		 * Panic path: acquire the doorbell register lock before
		 * clearing Command Complete below, which transfers
		 * ownership to the platform.  Once Command Complete is
		 * clear the doorbell must be rung, so the lock is held
		 * across both.  EBUSY here is retryable: Command
		 * Complete is still set, so we still own the channel.
		 * The Status field itself needs no lock: it lives in
		 * shared memory and is updated with atomic_and_16.
		 */
		ASSERT(pc->pc_db_reg != NULL);
		if (!mutex_tryenter(&pc->pc_db_reg->pre_lock)) {
			return (EBUSY);
		}
	}
	...
	if (trylock) {
		/*
		 * The doorbell lock is held, so this cannot fail: the
		 * channel is never left with ownership transferred and
		 * the doorbell unrung.
		 */
		pcc_reg_rmw_locked(pc->pc_db_reg, pc->pc_db_preserve,
			pc->pc_db_set);
		mutex_exit(&pc->pc_db_reg->pre_lock);
	}
...
pcc_type12_poll_complete(pcc_chan_t *pc, boolean_t *responded)
{
	...
	*responded = B_FALSE;
	for (;;) {
		if ((*status & PCC_STATUS_CMD_COMPLETE) != 0) {
			*responded = B_TRUE;
			return (DDI_SUCCESS);
		}
 ...
 * bit 0 (Command Complete) is set, PCC_IRQ_NOTIFY if bit 1
 * (Platform Interrupt) or bit 3 (Platform Notification) is set.
 * Does NOT clear anything; clearing is done by the handler
 * (bit 1, via the ack register path) and by
 * pcc_type12_clear_notify (bits 1 and 3).
 */
static int
pcc_type12_irq_check(pcc_chan_t *pc, pcc_irq_result_t *result)
{
	...
	*result = PCC_IRQ_NONE;
	...
	if ((val & (PCC_STATUS_PLATFORM_IRQ |
		PCC_STATUS_PLATFORM_NOTIFY)) != 0) {
...
/*
 * Clear the platform notification bits (1 and 3) in the Status
 * field after the notification callback has run.  Called from
 * pcc_notify_channel with pc_lock held.  Uses an interlocked
...
const pcc_type_ops_t pcc_type1_ops = {
	...
	.pto_init = NULL,   /* no post-map init for Type 1 */
...
const pcc_type_ops_t pcc_type2_ops = {
	.pto_parse = pcc_type12_parse,
	.pto_map = pcc_type12_map,
	.pto_init = NULL,   /* no post-map init for Type 2 */
	.pto_unmap = pcc_type12_unmap,
	.pto_prepare_send = pcc_type12_prepare_send,
	.pto_poll_complete = pcc_type12_poll_complete,
	.pto_check_error = pcc_type12_check_error,
