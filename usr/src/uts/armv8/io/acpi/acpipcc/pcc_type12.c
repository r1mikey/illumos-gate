	...
	uint32_t __unused payload_len, boolean_t trylock)
{
	 ...
	 * Completion (bit 15) iff we have an interrupt and this is
	 * not the panic path.  The platform generates the
	 * completion interrupt only if this bit is set; without it,
	 * interrupt-driven completion would never fire and every
	 * send would time out.  When polling (pc_has_irq false)
	 * the bit stays clear so the platform does not raise an
	 * interrupt nobody handles.  On the panic path (trylock)
	 * the bit also stays clear: a fire-and-forget send has no
	 * waiter for a completion interrupt.  pc_has_irq is false
	 * when the PCCT global Platform Interrupt flag is off,
	 * which satisfies Table 14.10's "must be cleared" rule.
	 *
	...
	*cmdfield = (uint16_t)cmd |
		((pc->pc_has_irq && !trylock) ? PCC_CMD_NOTIFY_ON_COMPLETE :
		0);
