...
/*
 * Shared memory header layouts (ACPI 6.6 Chapter 14).  Offsets are
 * relative to the start of the channel's shared memory region.
 * Access is always via volatile casts.  Private to the driver:
 * consumers stage payloads with the pcc_chan_write accessors and
...
typedef struct pcc_type_ops {
	...
	int	(*pto_prepare_send)(pcc_chan_t *pc, uint32_t cmd,
			uint32_t payload_len);
					/*
					 * Check ownership, populate the
					 * type-specific shared memory
					 * header, and clear Command Complete
					 * to transfer ownership to the
					 * platform.  The doorbell is rung
					 * by the caller after this
					 * returns; the implementation must
					 * not ring it again.
					 */
