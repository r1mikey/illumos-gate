...
#if XXXARM
	/* do we need to enable access to the ITS registers? */
        typer = ddi_get64(sc->sc_regh, GITS_REG64(sc, GITS_TYPER));
        if ((typer & GITS_TYPER_Physical) == 0) {
                dev_err(dip, CE_NOTE, "Device does not support physical LPIs.");
                ddi_regs_map_free(&sc->sc_regh);
                ddi_soft_state_free(gicv3its_soft_state, instance);
                return (DDI_FAILURE);
        }

        dev_err(dip, CE_CONT, "%lu device ID bits, %lu event ID bits\n",
            ((typer & GITS_TYPER_Devbits) >> GITS_TYPER_Devbits_SHIFT) + 1,
            ((typer & GITS_TYPER_ID_bits) >> GITS_TYPER_ID_bits_SHIFT) + 1);        
        
        /*
         * XXXARM: I _think_ GITS_CTLR.ITS_Number is the same as
         * illumos,translation-id, but I don't think it matters for now.
         */

        /*
         * GICR_PROPBASER.IDbits - number of bits of LPI space, -1 - so
         * 8092 + (2**IDBits) is the max LPI
         *
         * This is clamped to GICD_TYPER.IDbits.
         *
         * Even better is GICD_TYPER.num_LPIs [15:11] - iff 0, then use
         * GICD_TYPER.IDbits, else it's (2**(num_LPIs + 1)) and range is
         * 8192..(8192 + ((2**(num_LPIs + 1))-1)). The value yielded cannot
         * be an intid higher than GICD_TYPER.IDbits.
         *
         * When the supported INTID width is less than 14 bits, this field
         * is RES0 and no LPIs are supported.
         *
         * So, iff we have GICD_TYPER.num_LPIs, use it, otherwise...
         * Read GICR_PROPBASER.IDbits to get the maximum INTID and subtract 8192
         *
         * Finally, we need to be able to query the parent GIC to figure these
         * things out.
         */
#endif
	ddi_report_dev(dip);
