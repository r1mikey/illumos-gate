 ...
 *
 * The driver is a thin copyin/copyout shim over the kernel's efi_get_variable,
 * efi_set_variable, and efi_get_next_variable_name wrappers.  Those wrappers
 * handle all the heavy lifting, leaving only the interface to this driver.
 *
 * Privilege model:
 * - Reading variables (VAR_GET, VAR_NEXT) requires no special privilege.  UEFI
 *   variables are not secret; firmware exposes them freely to any caller.
 * - Writing variables (VAR_SET) requires PRIV_SYS_CONFIG.  Modifying boot
 *   variables or secure boot state is a privileged operation.
 */
...
/*
 * Maximum sizes for userland buffers.  These are sanity limits to prevent
 * userland from asking us to allocate unreasonable amounts of kernel memory.
 *
 * UEFI variable names are typically short (< 256 bytes).  The UEFI
 * specification does not mandate a maximum variable data size, but 32KB covers
 * all practical cases (boot entries, certificates, etc.).
 */
...
/*
 * Translate a UEFI status code to an errno value.
 */
static int
...
efidev_var_get(intptr_t arg, int mode)
{
	...
	if (status == EFI_BUFFER_TOO_SMALL) {
		...
		if (ddi_copyout(&ev, (void *)arg, sizeof (ev), mode) != 0) {
			rv = EFAULT;
		} else {
			rv = EOVERFLOW;
		}
	...
	/*
	 * Clamp the firmware-returned data_size to our allocation.  A
	 * well-behaved firmware returns data_size <= alloc_data_size on
	 * EFI_SUCCESS, but a buggy one could return more and we must not
	 * copyout past our buffer (kernel heap disclosure).
	 */
...
efidev_var_set(intptr_t arg, int mode, cred_t *cr)
{
	...
	name = kmem_alloc(ev.vi_name_size, KM_SLEEP);
	if (ddi_copyin(ev.vi_name, name, ev.vi_name_size, mode) != 0) {
		rv = EFAULT;
		goto out;
	}
	...
	if (ev.vi_data_size > 0) {
		...
		if (ddi_copyin(ev.vi_data, data, ev.vi_data_size, mode) != 0) {
			rv = EFAULT;
			goto out;
		}
	}
...
efidev_var_next(intptr_t arg, int mode)
{
	...
	if (ddi_copyin(ev.vi_name, name, alloc_name_size, mode) != 0) {
		rv = EFAULT;
		goto out;
	}
	...
	if (ddi_copyout(name, ev.vi_name, (size_t)name_size, mode) != 0) {
		rv = EFAULT;
		goto out;
	}
...
static int
efidev_open(dev_t *devp __unused, int flag __unused,
    int otyp, cred_t *cr __unused)
{
	if (otyp != OTYP_CHR) {
		return (EINVAL);
	}
...
static int
efidev_close(dev_t dev __unused, int flag __unused,
    int otyp __unused, cred_t *cr __unused)
{
	return (0);
}

static int
efidev_ioctl(dev_t dev __unused, int cmd, intptr_t arg, int mode,
    cred_t *cr, int *rvalp __unused)
{
	switch (cmd) {
	case EFIIOC_VAR_GET:
...
static int
efidev_info(dev_info_t *dip __unused, ddi_info_cmd_t cmd,
    void *arg __unused, void **resultp)
{
	switch (cmd) {
...
static struct cb_ops efidev_cb_ops = {
	.cb_open	= efidev_open,
	.cb_close	= efidev_close,
	.cb_strategy	= nodev,
	.cb_print	= nodev,
	.cb_dump	= nodev,
	.cb_read	= nodev,
	.cb_write	= nodev,
	.cb_ioctl	= efidev_ioctl,
	.cb_devmap	= nodev,
	.cb_mmap	= nodev,
	.cb_segmap	= nodev,
	.cb_chpoll	= nochpoll,
...
static struct dev_ops efidev_ops = {
	.devo_rev	= DEVO_REV,
	.devo_refcnt	= 0,
	.devo_getinfo	= efidev_info,
	.devo_identify	= nulldev,
	.devo_probe	= nulldev,
	.devo_attach	= efidev_attach,
	.devo_detach	= efidev_detach,
	.devo_reset	= nodev,
	.devo_cb_ops	= &efidev_cb_ops,
	.devo_bus_ops	= NULL,
	.devo_power	= nulldev,
...
static struct modldrv modldrv = {
	...
	.drv_linkinfo	= "UEFI variable access",
	.drv_dev_ops	= &efidev_ops
};
