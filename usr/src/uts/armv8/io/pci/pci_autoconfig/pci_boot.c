	...
	if (reprogram_mem) {
		...
		if (pci_bus_res[secbus].mem_avail != NULL &&
			...
			pci_memlist_insert(&pci_bus_res[parbus].mem_used,
			    mem.base, mem.size);
			(void) pci_memlist_remove(&pci_bus_res[parbus].mem_avail,
			    mem.base, mem.size);
		...
		if (pci_bus_res[secbus].pmem_avail != NULL &&
			...
			pci_memlist_insert(&pci_bus_res[parbus].pmem_used,
			    pmem.base, pmem.size);
			(void) pci_memlist_remove(&pci_bus_res[parbus].pmem_avail,
			    pmem.base, pmem.size);
			pci_bus_res[secbus].mem_reprogram = B_TRUE;
		} else {
			/* get new mem resource from parent bus */
...
pci_reprogram(void)
{
	...
	dcmn_err(CE_CONT, "Reprogramming PCI\n");

	/*
	 * Go through each bus and ask the platform if it wants to change the
	 * name of the slot.
	 *
	 * XXXPCI: We don't ask the PRD to rescan for root complexes, we
	 * currently rely utterly on firmware to have done initial PCI
	 * programming, and anything the PRD might find that firmware does not
	 * will by definition be completely unprogrammed.
	 *
	 * If we can switch to enumerating under the PCIe nexus and via
	 * pcicfg, we would instead do _everything_ via the PRD.
	 */
	...
	for (bus = 0; bus <= pci_boot_maxbus; bus++) {
		...
		if (pci_bus_res[bus].par_bus != (uchar_t)-1) {
			if (bus_debug(bus)) {
				dcmn_err(CE_CONT, "skipping discovery on peer bus %x\n", bus);
			}
			continue;
		} else {
			if (bus_debug(bus)) {
				dcmn_err(CE_CONT, "resource discovering bus %x\n", bus);
			}
		}
		 ...
		 */
		(void) pci_memlist_remove(&pci_bus_res[bus].mem_avail, 0, 0x100000);
...
enumerate_bus_devs(uchar_t bus, int config_op)
{
	...
	for (dev = 0; dev < max_dev_pci; dev++) {
		...
		for (func = 0; func < nfunc; func++) {
			ushort_t venid = pci_getw(bus, dev, func, PCI_CONF_VENID);
			ushort_t devid = pci_getw(bus, dev, func, PCI_CONF_DEVID);
