 ...
 * 5. Program the PCI device's MSI/MSI-X capability registers with the
 *    doorbell physical address and SPI number.
 *
 * For MSI, the PCI cap has a single shared address/data register pair.
 * We always program it with the base SPI and total count from the
 * per-device state, regardless of which inum is being enabled.  This
 * is idempotent and correct even when called multiple times (e.g., from
 * gicv2m_blockenable iterating per-handle).
 */
static int
 ...
 *
 * 1. Clear PCI MSI/MSI-X programming.
 * 2. Remove the handler via rem_avintr (triggers delspl which disables
...
/*
 * Block-enable all MSI vectors in a single operation.
 * MSI-only (not MSI-X).  The handle array is in ih_scratch2.
 */
static int
gicv2m_blockenable(gicv2m_state_t *sc, dev_info_t *rdip,
    ddi_intr_handle_impl_t *hdlp)
{
	int i;
	int count = hdlp->ih_scratch1;
	...
	for (i = 0; i < count; i++) {
		int ret = gicv2m_enable(sc, rdip, h_array[i]);
		if (ret != DDI_SUCCESS) {
			/* Undo enables for vectors 0..i-1 */
			for (int j = 0; j < i; j++)
				(void) gicv2m_disable(sc, rdip, h_array[j]);
			return (ret);
		}
	}
...
/*
 * Block-disable all MSI vectors in a single operation.
 */
static int
gicv2m_blockdisable(gicv2m_state_t *sc, dev_info_t *rdip,
    ddi_intr_handle_impl_t *hdlp)
{
	int count = hdlp->ih_scratch1;
	ddi_intr_handle_impl_t **h_array =
	...
	for (int i = 0; i < count; i++)
		(void) gicv2m_disable(sc, rdip, h_array[i]);
    ...
    ddi_intr_op_t intr_op, ddi_intr_handle_impl_t *hdlp, void *result)
{
	...
	case DDI_INTROP_BLOCKENABLE:
		return (gicv2m_blockenable(sc, rdip, hdlp));
	case DDI_INTROP_BLOCKDISABLE:
		return (gicv2m_blockdisable(sc, rdip, hdlp));
...
gicv2m_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 * wrong distributor.  Slave GICv2 is not supported.
	 */
