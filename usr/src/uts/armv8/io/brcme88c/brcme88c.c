...
mmc_sd_send_ocr(struct mmc_sc *sc)
{
	...
	union emmc_capabilities caps = {{sc->caps, sc->caps1}};
...
mmc_init(struct mmc_sc *sc)
{
	...
	sc->caps = mmc_reg_read(sc, EMMC_CAPABILITIES);
	sc->caps1 = mmc_reg_read(sc, EMMC_CAPABILITIES + 4);

	if ((fwcaps = ddi_prop_get_int64(DDI_DEV_T_ANY, sc->dip,
	    DDI_PROP_DONTPASS, "sdhci-caps-mask", 0)) != 0) {
		dev_err(sc->dip, CE_WARN, "Applying sdhci-caps-mask: 0x%lx",
		    fwcaps);
		uint32_t c = fwcaps & 0xffffffff;
		sc->caps &= (~c);
		c = (fwcaps >> 32) & 0xffffffff;
		sc->caps1 &= (~c);
	}

	if ((fwcaps = ddi_prop_get_int64(DDI_DEV_T_ANY, sc->dip,
	    DDI_PROP_DONTPASS, "sdhci-caps", 0)) != 0) {
		dev_err(sc->dip, CE_WARN, "Applying sdhci-caps: 0x%lx", fwcaps);
		uint32_t c = fwcaps & 0xffffffff;
		sc->caps |= c;
		c = (fwcaps >> 32) & 0xffffffff;
		sc->caps1 |= c;
	...
	/*
	 * We have no regulator control, so mask 3v0 and 1v8 capabilities.
	 */
	sc->caps &= ~(1 << 25);
	sc->caps &= ~(1 << 26);
