 ...
 *   - acpicpu: parses ACPI _LPI on ACPI0007 processor devices
 *   - fdtcpu:  parses DT idle-states on FDT cpu nodes
...
/* Per-CPU idle duration tracking, indexed by cpu_seqid */
static hrtime_t *lpi_last_idle;

/*
 * Per-CPU array of kstat pointers, one per idle state.
 *
 * Indexed as lpi_kstats[cpu_seqid][state_idx].
 */
 ...
 * not supply it (common in both ACPI and DT), the cpuidle framework
 * synthesises it so there is always a fallback.
 */
static void
 ...
 * shallowest state (WFI, residency 1us) and the highest index is the
 * deepest.  lpi_select_state() walks from deepest to shallowest, so
 * this ordering is required for correct state selection.
 */
static int
