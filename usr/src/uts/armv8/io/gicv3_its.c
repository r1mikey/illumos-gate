...
#define	GICV3_ITS_CMDQ_SIZE		(64 * 1024)	/* 64KB */
...
/*
 * ITS command: 32 bytes, 4 DWORDs.  Built on stack, bcopy'd to queue.
 */
...
static int gicv3_its_bus_ctl(dev_info_t *, dev_info_t *,
 ...
 *
 * Wraps the three-step DDI DMA allocation sequence for physically
 * contiguous, device-visible memory.  Used for the command queue,
 * device table, collection table and per-device ITTs.
 */

static int
gicv3_its_dma_alloc(dev_info_t *dip, size_t size, size_t align,
...
static void
gicv3_its_dma_free(ddi_dma_handle_t *dma_hdlp, ddi_acc_handle_t *acc_hdlp)
{
    ...
    uint8_t size, uint64_t itt_pa, boolean_t valid)
{
	...
	cmd->raw[0] = ((uint64_t)devid << 32) | GITS_CMD_MAPD;
	cmd->raw[1] = (uint64_t)(size & 0x1f);
	cmd->raw[2] = (valid ? (1ULL << 63) : 0) |
	    (itt_pa & GITS_BASERn_Physical_Address);
}
...
{
	bzero(cmd, sizeof (*cmd));
	cmd->raw[0] = (uint64_t)GITS_CMD_MAPC;
	if (pta) {
...
{
	bzero(cmd, sizeof (*cmd));
	cmd->raw[0] = ((uint64_t)devid << 32) | GITS_CMD_MAPTI;
	cmd->raw[1] = ((uint64_t)pintid << 32) | eventid;
	cmd->raw[2] = (uint64_t)collid;
}
...
{
	bzero(cmd, sizeof (*cmd));
	cmd->raw[0] = ((uint64_t)devid << 32) | GITS_CMD_MOVI;
	cmd->raw[1] = (uint64_t)eventid;
	cmd->raw[2] = (uint64_t)collid;
}
...
{
	bzero(cmd, sizeof (*cmd));
	cmd->raw[0] = ((uint64_t)devid << 32) | GITS_CMD_DISCARD;
	cmd->raw[1] = (uint64_t)eventid;
}
...
{
	bzero(cmd, sizeof (*cmd));
	cmd->raw[0] = ((uint64_t)devid << 32) | GITS_CMD_INV;
	cmd->raw[1] = (uint64_t)eventid;
}
...
{
	bzero(cmd, sizeof (*cmd));
	cmd->raw[0] = (uint64_t)GITS_CMD_SYNC;
	if (pta) {
		cmd->raw[2] = (target & 0xFFFFFFFFFULL) << 16;
	} else {
		cmd->raw[2] = (target & 0xFFFF) << 16;
	}
}
...
gicv3_its_cmd_submit(gicv3_its_state_t *sc, gicv3_its_cmd_t *cmd)
{
	...
	/* Check for stall before writing */
	creadr = its_read64(sc, GITS_CREADR);
	if (creadr & GITS_CREADR_Stalled) {
		dev_err(sc->its_dip, CE_WARN,
		    "command queue stalled at offset 0x%" PRIx64,
		    creadr & GITS_CREADR_Offset);
		return (EIO);
	}
    ...
    ddi_intr_handle_impl_t *hdlp, void *result)
{
	 ...
	 * We round up to GICV3_ITS_ITT_ALIGN for the hardware.
	 */
	...
	if (gicv3_its_dma_alloc(sc->its_dip, ds->ds_itt_sz,
	    GICV3_ITS_ITT_ALIGN, &ds->ds_itt, &ds->ds_itt_pa,
	...
	if (gicv3_its_do_mapd(sc, devid, ds->ds_itt_bits - 1,
		...
		gicv3_its_dma_free(&ds->ds_itt_dmah, &ds->ds_itt_acch);
		for (i = 0; i < actual; i++)
			gicv3_free_lpi(sc->its_gic_dip,
			    ds->ds_vecs[i].gv_lpi);
		kmem_free(ds->ds_vecs, ds->ds_vecs_sz);
    ...
    ddi_intr_handle_impl_t *hdlp)
{
	...
	/* Free LPIs back to the GICv3 parent's arena */
 ...
 * 2. Enable the LPI in the PROPBASER configuration table.
 * 3. Issue MAPTI + INV + SYNC to create the ITS translation.
 * 4. Register the handler via add_avintr (addspl guard skips distributor
 *    programming for LPI INTIDs >= 8192).
 * 5. Set ip_msi_addr/data on the handle for framework use.
 * 6. Program PCI MSI/MSI-X caps directly (transitional).
 */
static int
    ...
    ddi_intr_handle_impl_t *hdlp)
{
	 ...
	 * distributor programming for LPI INTIDs (>= 8192).
	 *
	...
	if (!add_avintr((void *)hdlp, hdlp->ih_pri,
		...
		(void) gicv3_its_do_discard(sc, ds->ds_devid, eventid,
		    target_cpu);
		gicv3_lpi_set_config(sc->its_gic_dip, lpi,
		    GIC_IPL_TO_PRIO(hdlp->ih_pri), B_FALSE);
		return (DDI_FAILURE);
	...
	/*
	 * PCI capability programming is handled by the DDI MSI framework
    ...
    ddi_intr_handle_impl_t *hdlp)
{
	...
	(void) gicv3_its_do_discard(sc, ds->ds_devid,
	    vec->gv_eventid, target_cpu);
...
/*
 * BLOCKENABLE: enable all MSI vectors in a handle array.
 */
static int
gicv3_its_blockenable(gicv3_its_state_t *sc, dev_info_t *rdip,
    ddi_intr_handle_impl_t *hdlp)
{
	int i;
	int count = hdlp->ih_scratch1;
	ddi_intr_handle_impl_t **h_array =
	    (ddi_intr_handle_impl_t **)hdlp->ih_scratch2;

	for (i = 0; i < count; i++) {
		int ret = gicv3_its_enable(sc, rdip, h_array[i]);
		if (ret != DDI_SUCCESS) {
			/* Undo enables for vectors 0..i-1 */
			for (int j = 0; j < i; j++)
				(void) gicv3_its_disable(sc, rdip,
				    h_array[j]);
			return (ret);
		}
	}
...
/*
 * BLOCKDISABLE: disable all MSI vectors in a handle array.
 */
static int
gicv3_its_blockdisable(gicv3_its_state_t *sc, dev_info_t *rdip,
    ddi_intr_handle_impl_t *hdlp)
{
	int count = hdlp->ih_scratch1;
	ddi_intr_handle_impl_t **h_array =
	...
	for (int i = 0; i < count; i++)
		(void) gicv3_its_disable(sc, rdip, h_array[i]);
    ...
    ddi_intr_handle_impl_t *hdlp)
{
	...
	new_cpu = (processorid_t)(uintptr_t)hdlp->ih_scratch1;
    ...
    ddi_intr_op_t intr_op, ddi_intr_handle_impl_t *hdlp, void *result)
{
	...
	case DDI_INTROP_BLOCKENABLE:
		return (gicv3_its_blockenable(sc, rdip, hdlp));
	case DDI_INTROP_BLOCKDISABLE:
		return (gicv3_its_blockdisable(sc, rdip, hdlp));
	case DDI_INTROP_SETTARGET:
		return (gicv3_its_settarget(sc, rdip, hdlp));
...
gicv3_its_pick_migration_cpu(processorid_t dying)
{
	cpu_t *cp;

	/*
	 * Walk the online CPU list.  Pick the first one that isn't
	 * the dying CPU.  cpu_active is safe to walk here because
	 * we are called under cpu_lock (from cpu_setup_func).
	 */
	cp = cpu_active;
	do {
		if (cp->cpu_id != dying)
...
gicv3_its_cpu_offline(gicv3_its_state_t *sc, processorid_t cpuid)
{
	...
	dest = gicv3_its_pick_migration_cpu(cpuid);
    ...
    ddi_dma_handle_t *dma_hdlp, ddi_acc_handle_t *acc_hdlp)
{
	...
	if (gicv3_its_dma_alloc(sc->its_dip, table_sz, page_sz,
	...
	*szp = table_sz;
	 ...
	 * non-shareable, retry with non-cacheable inner cache.
	 */
...
gicv3_its_init_cmdq(gicv3_its_state_t *sc)
{
	...
	if (gicv3_its_dma_alloc(sc->its_dip, sc->its_cmd_sz,
...
gicv3_its_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 * CPU callback atomically under cpu_lock (hma pattern).
	 *
	 * Holding cpu_lock ensures no CPU can come online between
	 * the iteration and callback registration - no gap.
	 *
	 * NOTE: CPU_SETUP for boot-time secondaries fires before
	 * the ITS driver loads, so the callback never sees them.
	 * The explicit iteration handles this.
	 */
	...
	dev_err(dip, CE_CONT,
	    ...
	    "PTA=%s doorbell PA 0x%" PRIx64 "\n",
	    sc->its_devbits, sc->its_idbits, sc->its_itt_entry_sz,
	    sc->its_hcc, sc->its_pta ? "yes" : "no",
	    sc->its_doorbell_pa);

	/* Register with parent GICv3 for MSI framework discovery */
	gicv3_register_its(sc->its_gic_dip, dip);
...
fail_tables:
	if (sc->its_devtab != NULL)
		gicv3_its_dma_free(&sc->its_devtab_dmah,
		    &sc->its_devtab_acch);
	if (sc->its_colltab != NULL)
		gicv3_its_dma_free(&sc->its_colltab_dmah,
		    &sc->its_colltab_acch);
	gicv3_its_dma_free(&sc->its_cmd_dmah, &sc->its_cmd_acch);
fail_regs:
...
/*
 * minimal nexus operations for DMA children
 */

static int
gicv3_its_bus_ctl(dev_info_t *dip, dev_info_t *rdip,
 ...
 *
 * bus_ops provides:
 *   - bus_intr_op for MSI framework routing
 *   - DMA passthrough ops so ITS children (if any) can allocate
 *     DMA memory (routes through to rootnex)
 *   - bus_ctl for INITCHILD/UNINITCHILD
 *   - bus_map/bus_map_fault for register mappings
 *   - bus_prop_op for property inheritance
 *
 * XXXMICHAEL: this is a bit questionable - we will not have children
 */
