 ...
 * MSI/MSI-X interrupts, and requires that GICD_TYPER.MBIS (i.e. the hardware
 * has MBI support), the device tree node has the "msi-controller" property
 * and one or more "mbi-ranges" base-and-bounds entries.  The optional
...
gicv3_mbi_init(dev_info_t *dip, gicv3_conf_t *gc)
{
	 ...
	 */
	{
		uint32_t base0 = (uint32_t)mbi_ranges[0];
		uint32_t count0 = (uint32_t)mbi_ranges[1];
    ...
    ddi_intr_op_t intr_op, ddi_intr_handle_impl_t *hdlp, void *result)
{
	 ...
	 */
	{
		gicv3_conf_t *mbi_gc = ddi_get_soft_state(gicv3_soft_state,
		    ddi_get_instance(dip));
