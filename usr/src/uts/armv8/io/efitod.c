...
/*
 * EFI Runtime Services Time-of-Day Driver.
 *
 * Implements the illumos tod_ops interface using UEFI Runtime Services
 * GetTime() and SetTime().  This driver is suitable for any  platform
...
/*
 * Set the firmware wakeup alarm to the specified time.
 * Must be called with tod_lock held.
 *
 * Converts the given timestruc_t (UTC seconds + nanoseconds) into an
 * EFI_TIME and programs the platform wakeup timer via SetWakeupTime().
 */
static void
efitod_set_power_alarm(timestruc_t ts)
{
	EFI_TIME	etime;
	todinfo_t	tod;
	...
	(void) efi_set_wakeup_time(1, &etime);
}

/*
 * Clear (disable) the firmware wakeup alarm.
 * Must be called with tod_lock held.
 */
static void
efitod_clear_power_alarm(void)
{
	ASSERT(MUTEX_HELD(&tod_lock));

	(void) efi_set_wakeup_time(0, NULL);
}
...
static struct modlmisc modlmisc = {
	...
	.misc_linkinfo = "UEFI Time-of-Day Clock",
};
...
_init(void)
{
	...
	if (strcmp(tod_module_name, "efitod") == 0) {
		...
		if (efirt_is_active()) {
			...
			tod_ops.tod_set_power_alarm = efitod_set_power_alarm;
			tod_ops.tod_clear_power_alarm =
			    efitod_clear_power_alarm;
		} else {
			cmn_err(CE_WARN,
