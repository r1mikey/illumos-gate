...
pcicfg_configure_ntbridge(dev_info_t *new_device, uint_t bus, uint_t device)
{
	...
	if (rc != NDI_SUCCESS) {
		cmn_err(CE_WARN,
		"pcicfg: Fail: cant load nontransparent bridge driver..\n");
		rc = PCICFG_FAILURE;
		return (rc);
	}
...
pcicfg_ntbridge_child(dev_info_t *dip)
{
	...
	if ((val = ddi_prop_get_int(DDI_DEV_T_ANY, ddi_get_parent(anode),
	    DDI_PROP_DONTPASS, PCI_DEV_CONF_MAP_PROP, -1)) != -1) {
...
{
	int		found = DDI_FAILURE, acount, i, ibridge;
	uint_t		length;
	...
	if (ddi_prop_lookup_int_array(DDI_DEV_T_ANY, dip, DDI_PROP_DONTPASS,
	    "assigned-addresses", (int **)&assigned, &length)
	    != DDI_PROP_SUCCESS) {
	...
	length = CELLS_1275_TO_BYTES(length);
...
pcicfg_device_assign_readonly(dev_info_t *dip)
{
	...
	if (ddi_prop_lookup_int_array(DDI_DEV_T_ANY, dip,
	    DDI_PROP_DONTPASS, "assigned-addresses", (int **)&assigned,
...
pcicfg_update_assigned_prop(dev_info_t *dip, pci_regspec_t *newone)
{
	...
	kmem_free(newreg, alen + sizeof (*newone));
	ddi_prop_free(assigned);
...
pcicfg_find_resource_end(dev_info_t *dip, void *hdl)
{
	...
	if (dip == entry->dip) {
		...
		if (ddi_prop_lookup_int_array(DDI_DEV_T_ANY, dip,
		    ...
		    (int **)&pci_ap,  &length) != DDI_PROP_SUCCESS) {
