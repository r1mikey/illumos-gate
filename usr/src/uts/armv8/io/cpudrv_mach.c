...
/*
 * Cached result of the DVFS availability probe.  Checked once via
 * double-checked locking in cpudrv_mach_dvfs_available() and reused
...
cpudrv_parse_one_state(dev_info_t *state_dip, lpi_state_t *lsp)
{
	...
	/*
	 * Context loss flags.  The DT binding doesn't carry explicit
	 * context loss bitmasks like ACPI _LPI - infer from available
	 * properties.
	 */
...
/*
 * Extract MPIDR from the FDT cpu node's reg property, respecting the
 * parent's #address-cells.
 */
static int
cpudrv_get_mpidr(dev_info_t *cpu_dip, uint64_t *mpidp)
{
	dev_info_t *cpus_dip;
	int addr_cells;
	int *reg;
	...
	cpus_dip = ddi_get_parent(cpu_dip);
	if (cpus_dip == NULL) {
		return (-1);
	}

	addr_cells = ddi_prop_get_int(DDI_DEV_T_ANY, cpus_dip,
	...
	if (addr_cells == 2 && reg_len >= 2) {
		*mpidp = ((uint64_t)(uint32_t)reg[0] << 32) |
		    (uint32_t)reg[1];
	} else if (addr_cells == 1 && reg_len >= 1) {
		*mpidp = (uint32_t)reg[0];
	} else {
		ddi_prop_free(reg);
		return (-1);
	}
...
cpudrv_get_cpu_id(dev_info_t *dip, processorid_t *cpu_id)
{
	...
	if (cpudrv_get_mpidr(dip, &mpidr) != 0) {
		return (B_FALSE);
	}
