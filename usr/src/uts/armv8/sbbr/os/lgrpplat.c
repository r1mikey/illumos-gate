 ...
 * lgrp_plat_set_acpi_tables() before lgrp_init(LGRP_INIT_STAGE1) runs.
...
/*
 * Lowest proximity domain ID seen in SRAT (for domain renumbering).
 */
...
/*
 * ACPI table pointers, set by lgrp_plat_set_acpi_tables() from mlsetup().
 */
...
static int	lgrp_plat_process_srat(ACPI_TABLE_SRAT *srat,
		    ACPI_TABLE_MSCT *msct, uint32_t *prox_domain_min,
 ...
 * Also tracks the minimum domain ID seen.
 * Returns the count, or -1 on error.
 */
static int
lgrp_plat_srat_domains(ACPI_TABLE_SRAT *srat, uint32_t *prox_domain_min)
{
	...
	while (item < end) {
		...
		if (domain != UINT32_MAX) {
			if (*prox_domain_min == UINT32_MAX ||
			    domain < *prox_domain_min)
				*prox_domain_min = domain;
 ...
 */
static int
lgrp_plat_msct_domains(ACPI_TABLE_MSCT *msct, uint32_t *prox_domain_min)
{
	if (msct == NULL)
		return (-1);
    ...
    memnode_phys_addr_map_t *memnode_info)
{
	 ...
	 */
	node_cnt = lgrp_plat_srat_domains(srat, prox_domain_min);
	if (node_cnt <= 0)
		return (-1);
	 ...
	 */
	msct_cnt = lgrp_plat_msct_domains(msct, prox_domain_min);
	...
	/*
	 * Count how many nodes were actually populated.
	 */
	node_cnt = 0;
    ...
    memnode_phys_addr_map_t *memnode_info)
{
	...
	for (i = 1; i < node_cnt; i++) {
		tmp = node_domain[i];
		j = i - 1;
		while (j >= 0 && node_domain[j].exists && tmp.exists &&
...
lgrp_plat_get_numa_config(void)
{
	...
	retval = lgrp_plat_process_srat(lgrp_plat_srat_ptr,
	    lgrp_plat_msct_ptr, &lgrp_plat_prox_domain_min,
	 ...
	 */
	max_mem_nodes = MAX_MEM_NODES_PER_LGROUP * lgrp_plat_node_cnt;
...
{
	if (from < 0 || from >= lgrp_plat_node_cnt ||
	    to < 0 || to >= lgrp_plat_node_cnt)
		return (0);
...
lgrp_plat_mem_size(lgrp_handle_t plathand, lgrp_mem_query_t query)
{
	...
	if (plathand == LGRP_DEFAULT_HANDLE && lgrp_plat_node_cnt == 1) {
	...
	for (mnode = 0; mnode < lgrp_plat_max_mem_node; mnode++) {
		...
		case LGRP_MEM_SIZE_FREE:
			/*
			 * TODO: Per-node free page accounting requires
			 * integration with the VM page counters
			 * (PLAT_Lx_xxx / vm_nfree[] or similar).
			 * For now, let the common code handle this.
			 */
			break;
...
plat_build_mem_nodes(struct memlist *list)
{
	...
	if (max_mem_nodes == 1 || lgrp_plat_node_cnt <= 1) {
		/*
		 * UMA system or single NUMA node: all memory belongs to
		 * memnode 0.  We must still register the memory ranges
		 * because the default path in startup_build_mem_nodes()
		 * does not run when plat_build_mem_nodes() is linked.
		 */
	...
	for (ml = list; ml != NULL; ml = ml->ml_next) {
		...
		/*
		 * Find which memnode this range belongs to and
		 * register it with the VM system.
		 */
		for (mnode = 0; mnode < lgrp_plat_max_mem_node; mnode++) {
			pfn_t ms, me, os, oe;
