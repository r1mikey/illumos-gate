...
{
	return (promif_nextnode(OBP_NONODE));
}
...
int
promif_bounded_getprop(pnode_t nodeid __unused, caddr_t name __unused,
    void *value __unused, int len __unused)
{
	return (-1);
}
...
void
promif_init(char *pgmname __unused, void *cookie __unused)
{
	/* nothing to do */
}

/*
 * This codepath reflects a compatability-only codepath from i86pc, where
 * somewhere between Solaris 2.6 and 10, we had a prom tree constructed by a
 * bootloader with realmode drivers. That is now gone, but we are left with
 * some applications depending on /dev/openprom. We fake a prom tree based on
 * hardware properties in the kernel device tree.
 *
 * The above, of course, only applies in this, the ACPI, case.
 */
void
promif_setup(void)
{
...
int
prom_fdt_get_clock_by_index(pnode_t node __unused, int index __unused,
    struct prom_hwclock *clock __unused)
{
	return (-1);
}

int
prom_fdt_get_clock_by_name(pnode_t node __unused, const char *name __unused,
    struct prom_hwclock *clock __unused)
{
	return (-1);
}
...
/* allows devfdt to load */
int
fdt_check_header(const void *fdt __unused)
{
	return (-9);	/* -FDT_ERR_BADMAGIC */
}
