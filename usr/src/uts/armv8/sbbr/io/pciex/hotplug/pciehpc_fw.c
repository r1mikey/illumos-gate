...
pciehpc_update_ops(pcie_hp_ctrl_t *ctrl_p)
{
	...
	uint32_t ctrl_req = PCIE_OSC_CONTROL_INIT |
	    PCIE_OSC_CTL_NAT_HP | PCIE_OSC_CTL_NAT_PM;
	uint32_t ctrl_ret = 0;

	/*
	 * Call _OSC method to determine if hotplug mode is native or ACPI.
	 * If _OSC method succeeds hp_native_mode below will be set according
	 * to if native hotplug control was granted or not by firmware.
	 *
	 * If _OSC method fails for any reason or if native hotplug control
	 * was not granted assume it's ACPI mode and update platform specific
	 * (ACPI) impl. ops.
	 */
	if (pcie_osc(ctrl_p->hc_dip, support, ctrl_req, &ctrl_ret) ==
	    DDI_SUCCESS) {
		hp_native_mode = (ctrl_ret & PCIE_OSC_CTL_NAT_HP) ?
		    B_TRUE : B_FALSE;
	}
