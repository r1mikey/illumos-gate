 ...
 *
 * On aarch64, PCIe root complexes are driven by "ecam" and appear at the
 * top of the devinfo tree.  Each ecam node's children are "pcieb" root
 * port bridges, structurally equivalent to the NPE → pcieb relationship
 * on i86pc.
 */
    ...
    topo_instance_t imin, topo_instance_t imax)
{
	...
	/*
	 * Scan for ecam driver instances — these are PCIe root complexes
	 * on aarch64.  Each ecam node's pcieb children are root ports.
	 */
	pnode = di_drv_first_node(ECAM, devtree);
